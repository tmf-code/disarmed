...
partial class Solve3D
{
...
)
  {
    ...
    if (error < acceptedError)
      return new SolveResult(links.Select((link) => Link.CopyLink(link)).ToArray(), () => error, true);

    if (joints.Length != links.Length + 1)
    {
      throw new Exception($"Joint transforms should have the same length as links + 1.Got { joints.Length }, expected {links.Length}");
    }
    ...
    Link[] withAngleStep = links.Select(
     (link, linkIndex) =>
     {
       ...
       var vectorComponents = new float[] { 0, 0, 0 }.Select((_, v3Index) =>
          {
            var eulerAngle = new float[] { 0, 0, 0 };
            eulerAngle[v3Index] = deltaAngle;
            var angleDelta = rotation * Quaternion.Euler(eulerAngle[0], eulerAngle[1], eulerAngle[2]);
            var linkWithAngleDelta = new Link(
            angleDelta,
            null,
            position
            );
            ...
            Link[] projectedLinks = new Link[] { linkWithAngleDelta }.Concat(links.ToList().Skip(linkIndex + 1)).ToArray();
            ...
            var angleStep =
                    -gradient * learningRate(projectedError);


            return angleStep;
          }).ToArray();

       var steppedRotation = rotation * Quaternion.Euler(vectorComponents[0], vectorComponents[1], vectorComponents[2]);
    ...
    return new SolveResult(withAngleStep,
   ...
   */
  public static JointTransformResult GetJointTransforms(
    Link[] links,
  ...
  )
  {
    ...
    for (var index = 0; index < links.Length; index++)
    {
      ...
      var absoluteRotation =
        parentTransform.rotation * currentLink.rotation;
  ...
  static Link ApplyConstraint(Link link, JointTransform joint)
  {
    ...
    if (constraints == null) return new Link(rotation, null, position);

    return constraints.Match(
      Right: new Func<ExactRotation, Link>(constraint =>
      {
