...
public class OVRLint : EditorWindow
{
  ...
  void OnGUI()
  {
    ...
    mShowRecordsStaticCommon = EditorGUILayout.BeginFoldoutHeaderGroup(
        mShowRecordsStaticCommon,
        $"Common Issues ({mRecordsStaticCommon.Count})",
    ...
    EditorGUILayout.BeginFoldoutHeaderGroup(
        false,
        "Quest Issues (disabled: Build Target is not Android)"
    );
    EditorGUILayout.EndFoldoutHeaderGroup();
    ...
    {
      mShowRecordsRuntimeCommon = EditorGUILayout.BeginFoldoutHeaderGroup(
          mShowRecordsRuntimeCommon,
          $"Common Runtime Issues ({mRecordsRuntimeCommon.Count})",
          EditorStyles.foldoutHeader
      );
    ...
    else
    {
      ...
      EditorGUILayout.BeginFoldoutHeaderGroup(
          false,
          "Common Runtime Issues (disabled: not in Play mode)"
      );
      EditorGUILayout.EndFoldoutHeaderGroup();
  ...
  void DisplayRecords(List<FixRecord> records)
  {
    for (int x = 0; x < records.Count; x++)
    {
      ...
      while (
          x + siblingRecordCount + 1 < records.Count
      ...
      ); //2-column wrapper for record
      EditorGUILayout.BeginVertical(GUILayout.Width(20)); //column 1: icon
      EditorGUILayout.LabelField(
          EditorGUIUtility.IconContent(record.complete ? "d_Progress" : "console.warnicon"),
          GUILayout.Width(20)
      );
      ...
      for (int i = 0; i <= siblingRecordCount; ++i)
      {
        ...
        if (iterRecord.targetObject)
          EditorGUILayout.ObjectField(
              iterRecord.targetObject,
              iterRecord.targetObject.GetType(),
              true
          );
      }
        ...
        for (int y = 0; y < record.buttonNames.Length; y++)
        {
          ...
          if (GUILayout.Button(record.buttonNames[y], EditorStyles.toolbarButton))
          {
            ...
            for (int i = 0; i <= siblingRecordCount; ++i)
            {
              ...
              {
                thisRecord.fixMethod(
                    thisRecord.targetObject,
                    (i == siblingRecordCount),
                    y
                );
  ...
  public static int RunCheck()
  {
    ...
    return mRecordsStaticCommon.Count
        + mRecordsStaticAndroid.Count
        + mRecordsRuntimeCommon.Count
  ...
  static void CheckStaticCommonIssues()
  {
    ...
    {
      AddFix(
          eRecordType.StaticCommon,
          "General",
          OVRManager.UnityAlphaOrBetaVersionWarningMessage,
          null,
          null,
          false
      );
    ...
    if (QualitySettings.pixelLightCount > recommendedPixelLightCount)
    {
      AddFix(
          eRecordType.StaticCommon,
          "Optimize Pixel Light Count",
          "For GPU performance set no more than "
              + recommendedPixelLightCount
              + " pixel lights in Quality Settings (currently "
...
#pragma warning disable 618
        if (!PlayerSettings.VROculus.dashSupport)
        {
            AddFix(
                eRecordType.StaticCommon,
                "Enable Dash Integration",
                "We recommend to enable Dash Integration for better user experience.",
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
        ...
        if (!PlayerSettings.VROculus.sharedDepthBuffer)
        {
            AddFix(
                ...
                "We recommend to enable Depth Buffer Sharing for better user experience on Oculus Dash.",
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
...
#pragma warning restore 618
    ...
    var tierSettings = UnityEditor.Rendering.EditorGraphicsSettings.GetTierSettings(
        target,
        tier
    );
    ...
    )
    {
      AddFix(
          eRecordType.StaticCommon,
          "Optimize Lightmap Directionality",
          ...
          null,
          false,
          "Switch to non-directional lightmaps"
      );
    }
    ...
    if (Lightmapping.realtimeGI)
    {
      AddFix(
          ...
          null,
          false,
          "Set Lightmapping.realtimeGI = false."
      );
    }
    ...
    for (int i = 0; i < lights.Length; ++i)
    {
      if (
          ...
          && !lights[i].bakingOutput.isBaked
          && IsLightBaked(lights[i])
      )
      {
        AddFix(
            eRecordType.StaticCommon,
    ...
    if (sources.Length > 16)
    {
      ...
      if (playingAudioSources.Count > 16)
      {
        ...
        playingAudioSources.Sort(
            delegate (AudioSource x, AudioSource y)
            {
    ...
    for (int i = 0; i < clips.Length; ++i)
    {
      if (clips[i].loadType == AudioClipLoadType.DecompressOnLoad)
      {
        AddFix(
            ...
            clips[i],
            false,
            "Change to Compressed in Memory",
            "Change to Streaming"
        );
      }
    ...
    {
      if (
          materials[i].shader.name.Contains("Parallax")
          || materials[i].IsKeywordEnabled("_PARALLAXMAP")
      )
      {
        AddFix(
            eRecordType.StaticCommon,
            ...
            delegate (UnityEngine.Object obj, bool last, int selected)
            {
              ...
              {
                var newName = thisMaterial.shader.name.Replace(
                            "-ParallaxSpec",
                            "-BumpSpec"
                        );
                ...
                {
                  Debug.LogWarning(
                              "Unable to find a replacement for shader "
                                  + materials[i].shader.name
                          );
                }
              }
            },
    ...
    for (int i = 0; i < renderers.Length; ++i)
    {
      if (renderers[i].sharedMaterial == null)
      {
        AddFix(
            eRecordType.StaticCommon,
            "Instanced Materials",
            "Please avoid instanced materials on renderers.",
            null,
            renderers[i],
            false
        );
  ...
  static void CheckRuntimeCommonIssues()
  {
    if (!OVRPlugin.occlusionMesh)
    {
      AddFix(
          ...
          null,
          false,
          "Set OVRPlugin.occlusionMesh = true"
      );
    }
