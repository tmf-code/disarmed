...
public class OVRSystemProfilerPanel : EditorWindow
{
  ...
  List<OVRSystemPerfMetrics.PerfMetrics> receivedMetricsList =
      new List<OVRSystemPerfMetrics.PerfMetrics>();
  ...
  void OnGUI()
  {
      ...
      if (GUILayout.Button("Start Server"))
      {
        ...
        if (adbTool.isReady)
        {
          ...
          EditorUtility.DisplayDialog(
              "ADB StartServer",
              (exitCode == 0 ? "Success" : "Failure. ExitCode = " + exitCode.ToString()),
              "Ok"
          );
        }
        else
    ...
    if (
        tcpClient.connectionState == OVRNetwork.OVRNetworkTcpClient.ConnectionState.Disconnected
    )
    {
    ...
    else
    {
      if (
          ...
          == OVRNetwork.OVRNetworkTcpClient.ConnectionState.Connecting
      )
      {
    ...
    lock (receivedMetricsList)
    {
      ...
      float? deviceCpuClockFrequencyInMHz = GetLatestPerfValueFloat(
          "deviceCpuClockFrequencyInMHz"
      );
      float? deviceGpuClockFrequencyInMHz = GetLatestPerfValueFloat(
  ...
  void GetMetricsField(
      string propertyName,
      out FieldInfo baseFieldInfo,
      out FieldInfo validalityFieldInfo
  )
  {
    baseFieldInfo = typeof(OVRSystemPerfMetrics.PerfMetrics).GetField(propertyName);
    validalityFieldInfo = typeof(OVRSystemPerfMetrics.PerfMetrics).GetField(
        propertyName + "_IsValid"
    );
  }
  ...
  int? GetLatestPerfValueInt(string propertyName)
  {
    ...
    if (
        baseFieldInfo == null
        || baseFieldInfo.FieldType != typeof(int)
        || (validalityFieldInfo != null && validalityFieldInfo.FieldType != typeof(bool))
    )
    {
  ...
  float? GetLatestPerfValueFloat(string propertyName)
  {
    ...
    if (
        baseFieldInfo == null
        || baseFieldInfo.FieldType != typeof(float)
        || (validalityFieldInfo != null && validalityFieldInfo.FieldType != typeof(bool))
    )
    {
  ...
  )
  {
    ...
    EditorGUI.ProgressBar(
        new Rect(r.x, r.y, barWidth, r.height),
        lastValue.Value,
    ...
    if (displayAverage)
    {
      ...
      {
        EditorGUI.ProgressBar(
            new Rect(r.x + barWidth + progressBarPad, r.y, barWidth, r.height),
            averageValue.Value,
            string.Format("{0:F1}%", averageValue.Value * 100.0f)
        );
      }
    }
    ...
    if (displayMaximum)
    {
      ...
      {
        EditorGUI.ProgressBar(
            new Rect(r.x + (barWidth + progressBarPad) * 2, r.y, barWidth, r.height),
            maxValue.Value,
            string.Format("{0:F1}%", maxValue.Value * 100.0f)
        );
      }
    }
  ...
  )
  {
    ...
    if (displayAverage)
    {
      ...
      if (averageValue.HasValue)
      {
        EditorGUI.ProgressBar(
            new Rect(r.x + barWidth + progressBarPad, r.y, barWidth, r.height),
            averageValue.Value / maxScale,
            string.Format("{0:F1} ms", averageValue.Value * 1000.0f)
        );
      }
    }
    ...
    if (displayMaximum)
    {
      ...
      if (maxValue.HasValue)
      {
        EditorGUI.ProgressBar(
            new Rect(r.x + (barWidth + progressBarPad) * 2, r.y, barWidth, r.height),
            maxValue.Value / maxScale,
            string.Format("{0:F1} ms", maxValue.Value * 1000.0f)
        );
      }
    }
  ...
  void OnPayloadReceived(int payloadType, byte[] buffer, int start, int length)
  {
    ...
    else
    {
      Debug.LogWarningFormat(
          "[OVRSystemProfilerPanel] unrecongized payload type {0}",
          payloadType
      );
    }
  }
  ...
  {
    Debug.LogFormat(
        "[OVRSystemPerfMetricsWindow] OnApplicationPlayModeStateChanged {0}",
        change.ToString()
    );
