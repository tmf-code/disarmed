...
public class OVRBundleTool : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (buildableScenes == null || buildableScenes.Count == 0)
        {
            ...
            var buildSettingBtnRt = GUILayoutUtility.GetRect(
                buildSettingsBtnTxt,
                GUI.skin.button,
                GUILayout.Width(150)
            );
        ...
        else
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                var sceneBtnRt = GUILayoutUtility.GetRect(
                    sceneBtnTxt,
                    GUI.skin.button,
                    GUILayout.Width(200)
                );
                ...
                GUIContent forceRestartLabel = new GUIContent(
                    "Force Restart [?]",
                    "Relaunch the application after scene bundles are finished deploying."
                );
                forceRestart = GUILayout.Toggle(
                    forceRestart,
                    forceRestartLabel,
        ...
        GUILayout.Space(10.0f);
        GUIContent transitionContent = new GUIContent(
            "Transition APK [?]",
            "Build and deploy an APK that will transition into the scene you are working on. This enables fast iteration on a specific scene."
        );
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            switch (currentApkStatus)
            {
                case ApkStatus.OK:
                    statusMesssage =
                        "<color=green>APK installed. Ready to build and deploy scenes.</color>";
                    break;
        ...
        if (showBundleManagement)
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                var clearDeviceBundlesBtnRt = GUILayoutUtility.GetRect(
                    clearDeviceBundlesTxt,
                    GUI.skin.button,
                    GUILayout.ExpandWidth(true)
                );
                ...
                var clearLocalBundlesBtnRt = GUILayoutUtility.GetRect(
                    clearLocalBundlesTxt,
                    GUI.skin.button,
                    GUILayout.ExpandWidth(true)
                );
        ...
        if (showOther)
        {
            EditorGUILayout.BeginHorizontal();
            {
                GUIContent useOptionalTransitionPackageLabel = new GUIContent(
                    "Use optional APK package name [?]",
                ...
                EditorGUILayout.LabelField(
                    useOptionalTransitionPackageLabel,
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                var launchBtnRt = GUILayoutUtility.GetRect(
                    launchBtnTxt,
                    GUI.skin.button,
                    GUILayout.ExpandWidth(true)
                );
                ...
                var uninstallBtnRt = GUILayoutUtility.GetRect(
                    uninstallTxt,
                    GUI.skin.button,
                    GUILayout.ExpandWidth(true)
                );
                ...
                var clearLogBtnRt = GUILayoutUtility.GetRect(
                    clearLogTxt,
                    GUI.skin.button,
                    GUILayout.ExpandWidth(true)
                );
        ...
        {
            debugLogScroll = EditorGUILayout.BeginScrollView(
                debugLogScroll,
            ...
            EditorGUILayout.SelectableLabel(
                toolLog,
                logBoxStyle,
                GUILayout.Height(logBoxSize.y + logBoxSpacing)
            );
            ...
            case GuiAction.BuildAndDeployScenes:
                ...
                if (
                    ...
                    && EditorUtility.DisplayDialog(
                        "Build and Deploy OVR Transition APK?",
    ...
    private static void GetScenesFromBuildSettings()
    {
        ...
        for (int i = 0; i < EditorBuildSettings.scenes.Length; i++)
        {
            ...
            if (scene.enabled)
            {
                ...
                {
                    EditorSceneInfo sceneInfo = new EditorSceneInfo(
                        scene.path,
    ...
    private static void CheckForTransitionAPK()
    {
        ...
        if (adbTool.isReady)
        {
            string matchedPackageList,
                error;
            var transitionPackageName = PlayerSettings.GetApplicationIdentifier(
            ...
            string[] packageCheckCommand = new string[]
            {
                "-d shell pm list package",
                transitionPackageName
            };
            if (
                adbTool.RunCommand(packageCheckCommand, null, out matchedPackageList, out error)
                == 0
            ) {
                ...
                else
                {
                    ...
                    if (matchedPackageList.Contains("package:" + transitionPackageName + "\r\n"))
                    {
                        ...
                        else
                        {
                            ...
                            string[] dumpPackageInfoCommand = new string[]
                            {
                                "-d shell dumpsys package",
                                transitionPackageName
                            };
                            string packageInfo;
                            if (
                                adbTool.RunCommand(
                                    dumpPackageInfoCommand,
                                    null,
                                    out packageInfo,
                                    out error
                                ) == 0
                                ...
                                && packageInfo.Contains(
                                    OVRBundleManager.TRANSITION_APK_VERSION_NAME
                                )
                            ) {
