...
public class DirectorySyncer
{
  ...
  private static string CheckedDirectory(string nameInExceptionText, string directory)
  {
    ...
    if (!Directory.Exists(directory))
    {
      throw new ArgumentException(
          string.Format(
              "{0} is not a valid directory for argument ${1}",
              directory,
              nameInExceptionText
          )
      );
  ...
  public DirectorySyncer(string source, string target, string ignoreRegExPattern = null)
  {
    ...
    )
    {
      throw new ArgumentException(
          string.Format(
              "Paths must not contain each other (source: {0}, target: {1}",
              Source,
              Target
          )
      );
  ...
  private HashSet<string> RelevantRelativeFilesBeneathDirectory(
      string path,
  ...
  )
  {
    return new HashSet<string>(
        Directory.GetFiles(path, "*", SearchOption.AllDirectories)
            ...
            .Select(p => PathHelper.MakeRelativePath(path, p))
            .Where(RelativeFilePathIsRelevant)
    );
  }

  private HashSet<string> RelevantRelativeDirectoriesBeneathDirectory(
      string path,
      CancellationToken cancellationToken
  )
  {
  ...
  private void DeleteOutdatedFilesFromTarget(
  ...
  private void DeleteOutdatedEmptyDirectoriesFromTarget(
  ...
  [SuppressMessage("ReSharper", "ParameterTypeCanBeEnumerable.Local")]
  private void CreateRelevantDirectoriesAtTarget(
      HashSet<string> sourceDirs,
      HashSet<string> targetDirs,
      CancellationToken cancellationToken
  )
  ...
  private void MoveRelevantFilesToTarget(
      SyncResult syncResult,
      CancellationToken cancellationToken
  )
  {
