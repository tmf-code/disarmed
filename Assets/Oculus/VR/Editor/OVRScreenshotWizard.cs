...
class OVRScreenshotWizard : ScriptableWizard
{
  ...
  void GenerateTexture(Cubemap cubemap, string pathName)
  {
    ...
    pathName = pathName.Replace(".cubemap", (textureFormat == TexFormat.PNG) ? ".png" : ".jpg")
        .ToLower();
    ...
    string fullPath = EditorUtility.SaveFilePanel(
        string.Format("Save Cubemap Screenshot as {0}", format),
        "",
        pathName,
        format.ToLower()
    );
  ...
  {
    OVRScreenshotWizard wizard = ScriptableWizard.DisplayWizard<OVRScreenshotWizard>(
        "OVR Screenshot Wizard",
        "Render Cubemap"
    );
    if (wizard != null)
    {
