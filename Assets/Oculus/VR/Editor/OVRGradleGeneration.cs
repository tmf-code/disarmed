...
#endif
{
  ...
  public void OnPreprocessBuild(BuildReport report)
  {
        ...
        if (PlayerSettings.Android.targetArchitectures != AndroidArchitecture.ARM64)
            throw new BuildFailedException(
                "Your project is using an Oculus XR Plugin version with known issues. Please navigate to the Package Manager and upgrade the Oculus XR Plugin to the latest verified version. When performing the upgrade"
    ...
    if (OculusBuildApp.GetBuildTelemetryEnabled())
    {
      ...
      OVRPlugin.AddCustomMetadata("target_platform", report.summary.platform.ToString());
            ...
            OVRPlugin.AddCustomMetadata(
                "scripting_runtime_version",
                UnityEditor.PlayerSettings.scriptingRuntimeVersion.ToString()
            );
#endif
      if (
          report.summary.platform == UnityEditor.BuildTarget.StandaloneWindows
  ...
  public void OnPostGenerateGradleAndroidProject(string path)
  {
    ...
    OVRPlugin.AddCustomMetadata(
        "target_oculus_platform",
        String.Join("_", targetOculusPlatform.ToArray())
    );
    UnityEngine.Debug.LogFormat(
        "QuestFamily = {0}: Quest = {1}, Quest2 = {2}",
    ...
    if (projectConfig != null && projectConfig.systemSplashScreen != null)
    {
      ...
      string splashScreenAssetPath = AssetDatabase.GetAssetPath(
      ...
      else
      {
        ...
        UnityEngine.Debug.LogFormat(
            "Copy splash screen asset from {0} to {1}",
            sourcePath,
            targetPath
        );
        try
    ...
    if (projectConfig != null)
    {
      if (projectConfig.enableNSCConfig)
      {
        ...
        try
        {
          ...
          File.Copy(
              securityConfigFile,
              Path.Combine(xmlDirectory, "network_sec_config.xml"),
              true
          );
  ...
  private static string GetOculusProjectNetworkSecConfigPath()
  {
    ...
    string configAssetPath = Path.GetFullPath(
        Path.Combine(editorDir, "network_sec_config.xml")
    );
  ...
  public void OnPostprocessBuild(BuildReport report)
  {
        ...
        if (autoIncrementVersion)
        {
            if ((report.summary.options & BuildOptions.Development) == 0)
            {
                ...
                UnityEngine.Debug.Log(
                    "Incrementing version code to " + PlayerSettings.Android.bundleVersionCode
                );
            }
        }
        ...
        {
            if (
                step.name.Contains("Compile scripts")
                ...
                OculusBuildApp.SendBuildEvent(
                    "build_step_" + step.name.ToLower().Replace(' ', '_'),
