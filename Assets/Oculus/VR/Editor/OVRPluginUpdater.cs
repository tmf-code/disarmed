...
class OVRPluginUpdater
{
  ...
  class PluginPackage
  {
    ...
    public Dictionary<PluginPlatform, string> Plugins = new Dictionary<
        PluginPlatform,
        string
    >();
    ...
    public bool IsWin64Enabled()
    {
      ...
      if (Plugins.TryGetValue(PluginPlatform.Win64, out path))
      {
        if (File.Exists(path))
        {
          ...
          if (pi != null)
          {
            return pi.GetCompatibleWithPlatform(BuildTarget.StandaloneWindows64)
                && pi.GetCompatibleWithEditor();
          }
        }
      }
            ...
            {
                {
                    ...
                    rootPath + GetPluginBuildTargetSubPath(PluginPlatform.Android)
                },
                {
                    PluginPlatform.AndroidUniversal,
                    rootPath + GetPluginBuildTargetSubPath(PluginPlatform.AndroidUniversal)
                },
                {
                    PluginPlatform.AndroidOpenXR,
                    rootPath + GetPluginBuildTargetSubPath(PluginPlatform.AndroidOpenXR)
                },
                {
                    PluginPlatform.OSXUniversal,
                    rootPath + GetPluginBuildTargetSubPath(PluginPlatform.OSXUniversal)
                },
                { PluginPlatform.Win, rootPath + GetPluginBuildTargetSubPath(PluginPlatform.Win) },
                {
                    PluginPlatform.Win64,
                    rootPath + GetPluginBuildTargetSubPath(PluginPlatform.Win64)
                },
                {
                    PluginPlatform.Win64OpenXR,
                    rootPath + GetPluginBuildTargetSubPath(PluginPlatform.Win64OpenXR)
                },
            }
    };
  }
    ...
    switch (target)
    {
      ...
      default:
        throw new ArgumentException(
            "Attempted GetPluginBuildTargetSubPath() for unsupported BuildTarget: " + target
        );
    }
  ...
  private static System.Version GetPluginVersion(string path)
  {
    ...
    if (pluginVersion == invalidVersion)
    {
      ...
      if (
          ...
          || pluginVersionInfo.ProductVersion == null
  ...
  private static void EnablePluginPackage(PluginPackage pluginPkg)
  {
      ...
      while (true)
      {
        ...
        else
        {
          UnityEngine.Debug.LogWarningFormat(
              "Unrecognized result from DisplayDialogComplex: {0}",
              result
          );
          break;
        }
    ...
    if (activateOpenXRPlugin)
    {
      ...
      if (!unityRunningInBatchmode)
      {
        EditorUtility.DisplayDialog(
            "OVRPlugin",
            "OVRPlugin with OpenXR backend will be activated by default",
            "Ok"
        );
      }
    }
    else
    {
      ...
      if (!unityRunningInBatchmode)
      {
        EditorUtility.DisplayDialog(
            "OVRPlugin",
            "OVRPlugin with LibOVR/VRAPI backend will be activated by default",
            "Ok"
        );
      }
    }
    ...
    foreach (var kvp in pluginPkg.Plugins)
    {
      ...
      if (
          (Directory.Exists(path + GetDisabledPluginSuffix()))
      ...
      )
      {
        ...
        switch (platform)
        {
          ...
          default:
            throw new ArgumentException(
                "Attempted EnablePluginPackage() for unsupported BuildTarget: "
                    + platform
            );
        }
  ...
  private static void AttemptPluginDisable()
  {
    ...
    foreach (PluginPackage pluginPkg in allUtilsPluginPkgs)
    {
      ...
      {
        if (
            (enabledUtilsPluginPkg == null)
            || (pluginPkg.Version > enabledUtilsPluginPkg.Version)
        )
        {
    ...
    {
      if (
          unityRunningInBatchmode
          || EditorUtility.DisplayDialog(
              "Disable Oculus Utilities Plugin",
      ...
      if (
          ...
          || EditorUtility.DisplayDialog(
              ...
              "Do you want to disable the OVRPlugin included with Oculus Utilities and revert to the OVRPlugin installed through the Package Manager?\n\n"
                  + "Current version: "
                  + GetVersionDescription(enabledUtilsPluginPkg.Version),
              "Yes",
              "No"
          )
      )
  ...
  private static void ActivateOVRPluginOpenXR()
  {
    if (!unityVersionSupportsAndroidUniversal)
    {
      UnityEngine.Debug.LogError(
          "Unexpected error: Unity must support AndroidUniversal version of Oculus Utilities Plugin for accessing OpenXR"
      );
      return;
    }
...
#if !USING_XR_SDK && !REQUIRES_XR_SDK
        UnityEngine.Debug.LogError(
            "Oculus Utilities Plugin with OpenXR only supports XR Plug-in Managmenent with Oculus XR Plugin"
        );
        return;
#else
    ...
    if (
        !enabledUtilsPluginPkg.IsAndroidOpenXRPresent()
        && !enabledUtilsPluginPkg.IsWin64OpenXRPresent()
    )
    {
      UnityEngine.Debug.LogError(
          "Unable to Activate OVRPlugin with OpenXR: Both AndroidOpenXR/OVRPlugin.aar or Win64OpenXR/OVRPlugin.dll does not exist"
      );
      return;
    }

    if (
        enabledUtilsPluginPkg.IsAndroidOpenXREnabled()
        && enabledUtilsPluginPkg.IsWin64OpenXREnabled()
    )
    {
      if (!unityRunningInBatchmode)
      {
        EditorUtility.DisplayDialog(
            "Unable to Activate OVRPlugin with OpenXR",
            "Both AndroidOpenXR/OVRPlugin.aar and Win64OpenXR/OVRPlugin.dll already enabled",
            "Ok"
        );
      }
      return;
    ...
    )
    {
      ...
      {
        string androidUniveralPluginPath = enabledUtilsPluginPkg.Plugins[
            PluginPlatform.AndroidUniversal
        ];
        string androidUniveralPluginBasePath = GetCurrentProjectPath();
        string androidUniveralPluginRelPath = androidUniveralPluginPath.Substring(
            androidUniveralPluginBasePath.Length + 1
        );
        PluginImporter pi =
            PluginImporter.GetAtPath(androidUniveralPluginRelPath) as PluginImporter;
        if (pi != null)
        {
          pi.SetCompatibleWithPlatform(BuildTarget.Android, false);
          AssetDatabase.ImportAsset(
              androidUniveralPluginRelPath,
        ...
        else
        {
          UnityEngine.Debug.LogWarning(
              "Unable to find PluginImporter: " + androidUniveralPluginRelPath
          );
        }
      }

      {
        string androidOpenXRPluginPath = enabledUtilsPluginPkg.Plugins[
            PluginPlatform.AndroidOpenXR
        ];
        string androidOpenXRPluginBasePath = GetCurrentProjectPath();
        string androidOpenXRPluginRelPath = androidOpenXRPluginPath.Substring(
            androidOpenXRPluginBasePath.Length + 1
        );
        PluginImporter pi =
            PluginImporter.GetAtPath(androidOpenXRPluginRelPath) as PluginImporter;
        if (pi != null)
        {
          pi.SetCompatibleWithPlatform(BuildTarget.Android, true);
          AssetDatabase.ImportAsset(
              androidOpenXRPluginRelPath,
              ImportAssetOptions.ForceUpdate
          );
        }
        else
        {
          UnityEngine.Debug.LogWarning(
              "Unable to find PluginImporter: " + androidOpenXRPluginRelPath
          );
        }
      }
    }
    ...
    )
    {
      if (enabledUtilsPluginPkg.IsWin64Enabled())
      {
        ...
        string win64PluginRelPath = win64PluginPath.Substring(
            win64PluginBasePath.Length + 1
        );
        ...
        {
          UnityEngine.Debug.LogWarning(
              "Unable to find PluginImporter: " + win64PluginRelPath
          );
        }
      }

      {
        string win64OpenXRPluginPath = enabledUtilsPluginPkg.Plugins[
            PluginPlatform.Win64OpenXR
        ];
        string win64OpenXRPluginBasePath = GetCurrentProjectPath();
        string win64OpenXRPluginRelPath = win64OpenXRPluginPath.Substring(
            win64OpenXRPluginBasePath.Length + 1
        );
        PluginImporter pi =
            PluginImporter.GetAtPath(win64OpenXRPluginRelPath) as PluginImporter;
        if (pi != null)
        {
          pi.SetCompatibleWithPlatform(BuildTarget.StandaloneWindows64, true);
          pi.SetCompatibleWithEditor(true);
        ...
        {
          UnityEngine.Debug.LogWarning(
              "Unable to find PluginImporter: " + win64OpenXRPluginRelPath
          );
        }
      }
    ...
    {
      EditorUtility.DisplayDialog(
          "Activate OVRPlugin with OpenXR",
          "Oculus Utilities Plugin with OpenXR has been enabled on Android",
          "Ok"
      );
      if (
          win64PluginUpdated
  ...
  private static void RestoreStandardOVRPlugin()
  {
    ...
    {
      UnityEngine.Debug.LogError(
          "Unable to Restore Standard Oculus Utilities Plugin: Oculus Utilities Plugin package not activated"
      );
      return;
    }

    if (
        !enabledUtilsPluginPkg.IsAndroidUniversalPresent()
        && !enabledUtilsPluginPkg.IsWin64Present()
    )
    {
      UnityEngine.Debug.LogError(
          "Unable to Restore Standard Oculus Utilities Plugin: Both AndroidOpenXR/OVRPlugin.aar and Win64/OVRPlugin.dll does not exist"
      );
      return;
    }

    if (
        enabledUtilsPluginPkg.IsAndroidUniversalEnabled()
    ...
    )
    {
      ...
      {
        EditorUtility.DisplayDialog(
            "Unable to Restore Standard Oculus Utilities Plugin",
            "Both AndroidUniversal/OVRPlugin.aar and Win64/OVRPlugin.dll already enabled",
            "Ok"
        );
      }
      return;
    ...
    if (!unityRunningInBatchmode)
    {
      EditorUtility.DisplayDialog(
          ...
          "Standard version of Oculus Utilities Plugin has been enabled on Android",
          "Ok"
      );
      if (
          win64PluginUpdated
  ...
  private static void AttemptPluginUpdate(bool triggeredByAutoUpdate)
  {
    ...
    {
      if (
          (newestUtilsPluginPkg == null) || (pluginPkg.Version > newestUtilsPluginPkg.Version)
      )
      {
    ...
    if (
        (newestUtilsPluginPkg != null)
        && (newestUtilsPluginPkg.Version > bundledPluginPkg.Version)
    )
    {
      if (
          (enabledUtilsPluginPkg == null)
          || (enabledUtilsPluginPkg.Version != newestUtilsPluginPkg.Version)
      )
      {
        ...
        (enabledUtilsPluginPkg != null)
        && (enabledUtilsPluginPkg.Version < bundledPluginPkg.Version)
    )
    {
      targetPluginPkg = bundledPluginPkg;
    }

    PluginPackage currentPluginPkg =
        (enabledUtilsPluginPkg != null) ? enabledUtilsPluginPkg : bundledPluginPkg;
    ...
    if (targetPluginPkg == null)
    {
      ...
      {
        EditorUtility.DisplayDialog(
            "Update Oculus Utilities Plugin",
    ...
    {
      string dialogBody =
          "Oculus Utilities has detected that a newer OVRPlugin is available."
      ...
      {
        dialogBody =
            "Oculus Utilities has detected a configuration change that requires re-enabling the current OVRPlugin."
  ...
  private static bool UnitySupportsEnabledAndroidPlugin()
  {
    ...
    foreach (PluginPackage pluginPkg in allUtilsPluginPkgs)
    {
      ...
      {
        if (
            (pluginPkg.IsAndroidUniversalEnabled() || pluginPkg.IsAndroidOpenXREnabled())
            && !unityVersionSupportsAndroidUniversal
        )
        {
          ...
          return false;
            ...
            !pluginPkg.IsAndroidUniversalEnabled()
            && pluginPkg.IsAndroidUniversalPresent()
            && !pluginPkg.IsAndroidOpenXREnabled()
            && pluginPkg.IsAndroidOpenXRPresent()
            && unityVersionSupportsAndroidUniversal
        )
        {
