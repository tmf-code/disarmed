...
namespace Assets.Oculus.VR.Editor
{
  public class OVRPlatformTool : EditorWindow
  {
    ...
    const string urlPlatformUtil = "https://www.oculus.com/download_app/?id=1076686279105243";
    ...
    void OnGUI()
    {
      ...
      OVRPlatformToolSettings.TargetPlatform = (TargetPlatform)MakeEnumPopup(
          TargetPlatformLabel,
      ...
      commandMenuScroll = EditorGUILayout.BeginScrollView(
          commandMenuScroll,
      ...
      );
      {
        ...
        // App ID
        GUIContent AppIDLabel = new GUIContent(
            "Oculus Application ID [?]: ",
            "This AppID will be used when uploading the build."
        );
        OVRPlatformToolSettings.AppID = MakeTextBox(
            AppIDLabel,
            OVRPlatformToolSettings.AppID
        );

        // App Token
        GUIContent AppTokenLabel = new GUIContent(
            "Oculus App Token [?]: ",
        ...
        // Release Channel
        GUIContent ReleaseChannelLabel = new GUIContent(
            "Release Channel [?]: ",
            "Specify the releaes channel of the new build, you can reassign to other channels after upload."
        );
        OVRPlatformToolSettings.ReleaseChannel = MakeTextBox(
            ReleaseChannelLabel,
            OVRPlatformToolSettings.ReleaseChannel
        );

        // Releaes Note
        GUIContent ReleaseNoteLabel = new GUIContent("Release Note: ");
        OVRPlatformToolSettings.ReleaseNote = MakeTextBox(
            ReleaseNoteLabel,
            OVRPlatformToolSettings.ReleaseNote
        );

        // Platform specific fields
        if (OVRPlatformToolSettings.TargetPlatform == TargetPlatform.Rift)
        {
          GUIContent BuildDirLabel = new GUIContent(
              "Rift Build Directory [?]: ",
              "The full path to the directory containing your Rift build files."
          );
          OVRPlatformToolSettings.RiftBuildDirectory = MakeFileDirectoryField(
              BuildDirLabel,
              OVRPlatformToolSettings.RiftBuildDirectory,
              "Choose Rifle Build Directory"
          );

          GUIContent BuildVersionLabel = new GUIContent(
              "Build Version [?]: ",
              "The version number shown to users."
          );
          OVRPlatformToolSettings.RiftBuildVersion = MakeTextBox(
              BuildVersionLabel,
              OVRPlatformToolSettings.RiftBuildVersion
          );

          GUIContent LaunchFileLabel = new GUIContent(
              "Launch File Path [?]: ",
              "The full path to the executable that launches your app."
          );
          OVRPlatformToolSettings.RiftLaunchFile = MakeFileDirectoryField(
              LaunchFileLabel,
        ...
        {
          GUIContent ApkPathLabel = new GUIContent(
              "Build APK File Path [?]: ",
              "The full path to the APK file."
          );
          OVRPlatformToolSettings.ApkBuildPath = MakeFileDirectoryField(
              ApkPathLabel,
              OVRPlatformToolSettings.ApkBuildPath,
              "Choose APK File",
              true,
              "apk"
          );
        ...
        GUIContent uploadDebugSymbolsLabel = new GUIContent(
            "Upload Debug Symbols [?]: ",
            "Check this box to enable uploading of debug symbols."
        );
        OVRPlatformToolSettings.UploadDebugSymbols = MakeToggleBox(
            uploadDebugSymbolsLabel,
            OVRPlatformToolSettings.UploadDebugSymbols
        );
          ...
          if (OVRPlatformToolSettings.TargetPlatform == TargetPlatform.Quest)
          {
            if (showUploadDebugSymbols != OVRPlatformToolSettings.UploadDebugSymbols)
            {
              ...
              if (string.IsNullOrEmpty(OVRPlatformToolSettings.DebugSymbolsDirectory))
              {
                ...
                {
                  OVRPlatformToolSettings.DebugSymbolsDirectory = Path.Combine(
            ...
            GUIContent DebugSymbolLabel = new GUIContent(
                "Debug Symbols Directory [?]: ",
                "The full path to the directory containing the app symbols (libil2cpp.sym.so)"
            );
            OVRPlatformToolSettings.DebugSymbolsDirectory = MakeFileDirectoryField(
                DebugSymbolLabel,
                OVRPlatformToolSettings.DebugSymbolsDirectory,
                "Choose Debug Symbols Directory",
                false
            );

            GUIContent uploadSymbolsOnlyLabel = new GUIContent(
                "Upload Debug Symbols Only [?]: ",
            ...
            OVRPlatformToolSettings.UploadDebugSymbolsOnly = MakeToggleBox(
                uploadSymbolsOnlyLabel,
                OVRPlatformToolSettings.UploadDebugSymbolsOnly
            );

            if (OVRPlatformToolSettings.UploadDebugSymbolsOnly)
            {
              GUIContent BuildIDLabel = new GUIContent(
                  "Build ID [?]: ",
                  "This BuildID will be used for uploading debug symbols to a specific build."
              );
              OVRPlatformToolSettings.BuildID = MakeTextBox(
                  BuildIDLabel,
                  OVRPlatformToolSettings.BuildID
              );
            }
          }
          DecrementIndent();
        ...
        showOptionalCommands = EditorGUILayout.Foldout(
        ...
        if (showOptionalCommands)
        {
          ...
          {
            // Launch Parameters
            GUIContent LaunchParamLabel = new GUIContent(
                "Launch Parameters [?]: ",
                "Specifies any arguments passed to the launcher."
            );
            OVRPlatformToolSettings.RiftLaunchParams = MakeTextBox(
                LaunchParamLabel,
                OVRPlatformToolSettings.RiftLaunchParams
            );

            GUIContent FirewallExceptionLabel = new GUIContent(
                "Firewall Exception [?]: ",
                "Specifies if a Windows Firewall exception is required."
            );
            OVRPlatformToolSettings.RiftFirewallException = MakeToggleBox(
                FirewallExceptionLabel,
                OVRPlatformToolSettings.RiftFirewallException
            );

            GUIContent GamepadEmulationLabel = new GUIContent(
                "Gamepad Emulation [?]: ",
                "Specifies the type of gamepad emulation used by the Oculus Touch controllers."
            );
            OVRPlatformToolSettings.RiftGamepadEmulation = (GamepadType)MakeEnumPopup(
                GamepadEmulationLabel,
                OVRPlatformToolSettings.RiftGamepadEmulation
            );

            show2DCommands = EditorGUILayout.Foldout(
                show2DCommands,
                "2D",
                boldFoldoutStyle
            );
            if (show2DCommands)
            {
              ...
              // 2D Launch File
              GUIContent LaunchFile2DLabel = new GUIContent(
                  "2D Launch File [?]: ",
                  "The full path to the executable that launches your app in 2D mode."
              );
              OVRPlatformToolSettings.Rift2DLaunchFile = MakeFileDirectoryField(
                  LaunchFile2DLabel,
                  OVRPlatformToolSettings.Rift2DLaunchFile,
              ...
              GUIContent LaunchParam2DLabel = new GUIContent(
                  "2D Launch Parameters [?]: ",
                  "Specifies any arguments passed to the launcher in 2D mode."
              );
              OVRPlatformToolSettings.Rift2DLaunchParams = MakeTextBox(
                  LaunchParam2DLabel,
                  OVRPlatformToolSettings.Rift2DLaunchParams
              );

              DecrementIndent();
            }

            showRedistCommands = EditorGUILayout.Foldout(
                showRedistCommands,
            ...
            if (showRedistCommands)
            {
              ...
              for (
                  int i = 0;
                  i < OVRPlatformToolSettings.RiftRedistPackages.Count;
                  i++
              )
              {
                GUIContent RedistPackageLabel = new GUIContent(
                    OVRPlatformToolSettings.RiftRedistPackages[i].name
                );
                OVRPlatformToolSettings.RiftRedistPackages[i].include =
                    MakeToggleBox(
                        RedistPackageLabel,
                        OVRPlatformToolSettings.RiftRedistPackages[i].include
                    );
            ...
            showExpansionFileCommands = EditorGUILayout.Foldout(
                showExpansionFileCommands,
                "Expansion Files",
                boldFoldoutStyle
            );
            if (showExpansionFileCommands)
            {
              ...
              GUIContent LanguagePackLabel = new GUIContent(
                  "Language Pack Directory [?]: ",
                  "The full path to the directory containing the language packs"
              );
              OVRPlatformToolSettings.LanguagePackDirectory = MakeFileDirectoryField(
                  LanguagePackLabel,
                  OVRPlatformToolSettings.LanguagePackDirectory,
          ...
          else
          {
            ...
            if (showExpansionFileCommands)
            {
              ...
              // OBB File Path
              GUIContent ObbPathLabel = new GUIContent(
                  "OBB File Path [?]: ",
                  "The full path to the OBB file."
              );
              OVRPlatformToolSettings.ObbFilePath = MakeFileDirectoryField(
                  ObbPathLabel,
                  OVRPlatformToolSettings.ObbFilePath,
                  "Choose OBB File",
                  true,
                  "obb"
              );
          ...
          {
            // Assets Directory
            GUIContent AssetsDirLabel = new GUIContent(
                "Assets Directory [?]: ",
                "The full path to the directory with DLCs for this build."
            );
            string assetsDirectory = MakeFileDirectoryField(
                AssetsDirLabel,
                OVRPlatformToolSettings.AssetsDirectory,
            ...
            if (assetsDirectory != OVRPlatformToolSettings.AssetsDirectory)
            {
              ...
              if (!string.IsNullOrEmpty(OVRPlatformToolSettings.AssetsDirectory))
              {
                ...
                {
                  OVRPlatformToolSettings.AssetConfigs.Add(
                      new AssetConfig(f.Name)
                  );
                }
              }
              EditorUtility.SetDirty(OVRPlatformToolSettings.Instance);
            ...
            Rect rect = GUILayoutUtility.GetRect(
                0,
                GetAssetConfigElementHeight() + (ASSET_CONFIG_BACKGROUND_PADDING * 2),
      ...
      // Run OVR Lint Option
      EditorGUIUtility.labelWidth = DEFAULT_LABEL_WIDTH;
      GUIContent RunOvrLintLabel = new GUIContent(
          "Run OVR Lint (Recommended) [?]: ",
          "Run OVR Lint tool to ensure project is optimized for performance and meets Oculus packaging requirement for publishing."
      );
      OVRPlatformToolSettings.RunOvrLint = MakeToggleBox(
          RunOvrLintLabel,
          OVRPlatformToolSettings.RunOvrLint
      );
      ...
      rt.center = new Vector2(
          EditorGUIUtility.currentViewWidth / 2 - rt.width / 2 - buttonPadding,
          btnYPos
      );
      ...
      logBoxStyle.normal.textColor = logBoxStyle.focused.textColor =
      ...
      EditorGUILayout.LabelField(
          "<b>Oculus Developer Hub</b> is a desktop companion tool that can upload builds, manage apps and reduce friction in daily Quest development.",
          odhCalloutStyle
      );
...
#if UNITY_EDITOR_WIN
        Application.OpenURL(
            "https://developer.oculus.com/downloads/package/oculus-developer-hub-win/?source=unity"
        );
#elif UNITY_EDITOR_OSX
                Application.OpenURL(
                    "https://developer.oculus.com/downloads/package/oculus-developer-hub-mac/?source=unity"
                );
#endif
      }
    ...
    private void SetOVRProjectConfig(TargetPlatform targetPlatform)
    {
            ...
            if (
                targetPlatform == TargetPlatform.Quest
    ...
    private void IncrementIndent()
    {
      ...
      EditorGUIUtility.labelWidth =
          DEFAULT_LABEL_WIDTH - (EditorGUI.indentLevel * INDENT_SPACING);
    }
    ...
    static void ExecuteCommand(TargetPlatform targetPlatform)
    {
      ...
      if (
          !ranSelfUpdate
          && File.Exists(dataPath + "/Oculus/VR/Editor/Tools/ovr-platform-util.exe")
      )
      {
        ranSelfUpdate = true;
        activeProcess = true;
    ...
    private static string CheckForPlatformUtil(string dataPath)
    {
      ...
      if (!System.IO.File.Exists(platformUtil))
      {
        ...
        EditorCoroutine downloadCoroutine = EditorCoroutine.Start(
            ProvisionPlatformUtil(platformUtil)
        );
    ...
    private static bool genUploadCommand(TargetPlatform targetPlatform, out string command)
    {
      ...
      if (targetPlatform == TargetPlatform.Rift)
      {
        ...
        {
          ValidateTextField(
              LaunchParameterValidator,
              OVRPlatformToolSettings.RiftLaunchParams,
              "Launch Parameters",
              ref success
          );
          command +=
              " --launch_params \"" + OVRPlatformToolSettings.RiftLaunchParams + "\"";
        }
        ...
        if (!string.IsNullOrEmpty(OVRPlatformToolSettings.Rift2DLaunchFile))
        {
          ...
          command +=
              " --launch_file_2d \"" + OVRPlatformToolSettings.Rift2DLaunchFile + "\"";
        ...
        if (redistCommandIds.Count > 0)
        {
          command +=
              " --redistributables \""
              + string.Join(",", redistCommandIds.ToArray())
              + "\"";
        }

        // Add Gamepad Emulation
        if (
            OVRPlatformToolSettings.RiftGamepadEmulation > GamepadType.OFF
        ...
        )
        {
          ...
          {
            case GamepadType.TWINSTICK:
              command += "TWINSTICK";
              break;
            case GamepadType.RIGHT_D_PAD:
              command += "RIGHT_D_PAD";
              break;
            case GamepadType.LEFT_D_PAD:
              command += "LEFT_D_PAD";
              break;
            default:
              command += "OFF";
      ...
      if (!string.IsNullOrEmpty(OVRPlatformToolSettings.AssetsDirectory))
      {
        ...
        if (OVRPlatformToolSettings.AssetConfigs.Count > 0)
        {
          ...
          for (int i = 0; i < OVRPlatformToolSettings.AssetConfigs.Count; i++)
          {
            ...
            if (configParameters.Count > 0)
            {
              string configString =
                  ...
                  + config.name
                  + "\\\":{"
                  + string.Join(",", configParameters.ToArray())
                  + "}";
          ...
          if (assetConfigs.Count > 0)
          {
            command +=
                " --asset_files_config {"
                + string.Join(",", assetConfigs.ToArray())
                + "}";
          }
        }
      }
    ...
    private static bool genDebugSymbolsCommand(out string command)
    {
      ...
      command +=
          " --debug-symbols-dir \"" + OVRPlatformToolSettings.DebugSymbolsDirectory + "\"";
      command += " --debug-symbols-pattern \"*.sym.so\"";
    ...
    private delegate TSuccess FieldValidatorDelegate<in TText, TError, out TSuccess>(
        TText text,
        ref TError error
    );
    ...
    static void ValidateTextField(
        FieldValidatorDelegate<string, string, bool> fieldValidator,
        string fieldText,
        string fieldName,
        ref bool success
    )
    {
      ...
      {
        OVRPlatformTool.log +=
            "ERROR: Please verify that the " + fieldName + " is correct. ";
    ...
    private static bool ThrowPlatformUtilStartupError(string utilPath)
    {
      if (retryCount < MAX_DOWNLOAD_RETRY_COUNT)
      {
        retryCount++;
        OVRPlatformTool.log +=
            "There was a problem starting Oculus Platform Util. Restarting provision process...\n";
      ...
      {
        OVRPlatformTool.log +=
            "OVR Platform Tool had a problem with downloading a valid executable after several trys. Please reopen the tool to try again.\n";
        return false;
      }
    }
    ...
    private string MakeFileDirectoryField(
        GUIContent label,
        string variable,
        string title,
        bool isFile = false,
    ...
    private static void DrawAssetConfigElement(Rect rect)
    {
      Rect elementRect = new Rect(
          rect.x,
          rect.y + SINGLE_LINE_SPACING + ASSET_CONFIG_BACKGROUND_PADDING / 2,
          rect.width,
          SINGLE_LINE_SPACING
      );
      if (OVRPlatformToolSettings.AssetConfigs.Count > 0)
      {
        for (int i = 0; i < OVRPlatformToolSettings.AssetConfigs.Count; i++)
        {
          ...
          config.SetFoldoutState(
              EditorGUI.Foldout(
                  elementRect,
                  config.GetFoldoutState(),
                  config.name,
                  boldFoldoutStyle
              )
          );
          ...
          {
            Rect attributeRect = new Rect(
                elementRect.x + INDENT_SPACING,
                elementRect.y + SINGLE_LINE_SPACING,
                elementRect.width - INDENT_SPACING - 3f,
                SINGLE_LINE_SPACING
            );
            // Extra asset config params are disabled for now until CLI supports them
#if !DISABLE_EXTRA_ASSET_CONFIG
            fieldLabel = new GUIContent(
                "Required Asset [?]",
                "Whether or not this asset file is required for the app to run."
            );
            config.required = EditorGUI.Toggle(
                attributeRect,
                fieldLabel,
                config.required
            );
            ...
            config.type = (AssetConfig.AssetType)EditorGUI.EnumPopup(
                attributeRect,
                fieldLabel,
                config.type
            );
            ...
            fieldLabel = new GUIContent(
                "Asset SKU [?]",
                "The Oculus store SKU for this asset file."
            );
      ...
      else
      {
        EditorGUI.LabelField(
            elementRect,
            "No asset files found. Choose a valid assets directory."
        );
      }
    }
    ...
    private static float GetAssetConfigElementHeight()
    {
      ...
      if (OVRPlatformToolSettings.AssetConfigs.Count > 0)
      {
        for (int i = 0; i < OVRPlatformToolSettings.AssetConfigs.Count; i++)
        {
          AssetConfig config = OVRPlatformToolSettings.AssetConfigs[i];
#if !DISABLE_EXTRA_ASSET_CONFIG
          totalHeight += config.GetFoldoutState()
              ? SINGLE_LINE_SPACING * 4
              : SINGLE_LINE_SPACING;
#else
                    totalHeight += config.GetFoldoutState()
                        ? SINGLE_LINE_SPACING * 2
                        : SINGLE_LINE_SPACING;
#endif
        }
      }
    ...
    private static void DrawAssetConfigBackground(Rect rect)
    {
      Rect backgroundRect = new Rect(
          rect.x,
          rect.y + SINGLE_LINE_SPACING,
          rect.width,
          GetAssetConfigElementHeight()
      );
      EditorGUI.DrawRect(
          backgroundRect,
          EditorGUIUtility.isProSkin
