...
public class OVRBundleManager
{
    ...
    public static void BuildDeployTransitionAPK(bool useOptionalTransitionApkPackage)
    {
        ...
        if (String.IsNullOrEmpty(transitionScenePath))
        {
            ...
            string[] res = System.IO.Directory.GetFiles(
        ...
        OculusBuildApp.SendBuildEvent(
            "oculus_bundle_tool",
            "apk_build_time",
            (DateTime.Now - apkBuildStart).TotalSeconds.ToString()
        );
    ...
    private static void PrebuildProjectSettingUpdate()
    {
        ...
        projectDefaultAppIdentifier = PlayerSettings.GetApplicationIdentifier(
        ...
        projectScriptImplementation = PlayerSettings.GetScriptingBackend(
            EditorUserBuildSettings.selectedBuildTargetGroup
        );
        projectManagedStrippingLevel = PlayerSettings.GetManagedStrippingLevel(
            BuildTargetGroup.Android
        );
    ...
    public static void BuildDeployScenes(
        List<OVRBundleTool.EditorSceneInfo> sceneList,
        bool forceRestart
    ) {
        externalSceneCache =
    ...
    private static void BuildSceneBundles(List<OVRBundleTool.EditorSceneInfo> sceneList)
    {
        ...
        {
            var resourceDirectories = Directory.GetDirectories(
                    "Assets",
                    "Resources",
                    SearchOption.AllDirectories
                )
            ...
            string[] resourceAssetPaths = AssetDatabase.FindAssets("", resourceDirectories)
                .Select(x => AssetDatabase.GUIDToAssetPath(x))
                .ToArray();
            ProcessAssets(
                resourceAssetPaths,
                "resources",
                ref uniqueAssetInSceneBundle,
                ref extToAssetList
            );
        ...
        string sceneBundleDirectory = Path.Combine(
            BUNDLE_MANAGER_OUTPUT_PATH,
        ...
        foreach (var scene in sceneList)
        {
            ...
            Debug.Log(
                "[OVRBundleManager] - "
                    + scene.sceneName
                    + " - Calculated scene asset dependencies in: "
        ...
        foreach (string ext in extToAssetList.Keys)
        {
            ...
            for (int i = 0; i < numChunks; i++)
            {
                ...
                {
                    assetChunkList = extToAssetList[ext].GetRange(
                        i * BUNDLE_CHUNK_SIZE,
                        BUNDLE_CHUNK_SIZE
                    );
                }
        ...
        Debug.Log(
            "[OVRBundleManager] - Created chucked scene bundles in: "
                + (DateTime.Now - labelingStart).TotalSeconds
        );

        // Build asset bundles
        BuildPipeline.BuildAssetBundles(
            sceneBundleDirectory,
        ...
        foreach (string asset in assetPaths)
        {
            ...
            if (!string.IsNullOrEmpty(ext))
            {
                ...
                if (!ext.Equals("cs") && !ext.Equals("unity"))
                {
                    ...
                    {
                        var assetObject = (UnityEngine.Object)AssetDatabase.LoadAssetAtPath(
                            asset,
                            typeof(UnityEngine.Object)
                        );
                        if (
                            assetObject == null
                            || (assetObject.hideFlags & HideFlags.DontSaveInBuild) == 0
                        ) {
    ...
    private static bool DeploySceneBundles(List<OVRBundleTool.EditorSceneInfo> sceneList)
    {
        ...
        string absoluteTempPath = Path.Combine(
        ...
        if (adbTool.isReady)
        {
            ...
            string sceneLoadDataPath = Path.Combine(
                tempDirectory,
                OVRSceneLoader.sceneLoadDataName
            );
            ...
            long unixTime = (int)(
                DateTimeOffset.UtcNow.ToUniversalTime() - new DateTime(1970, 1, 1, 0, 0, 0)
            ).TotalSeconds;
            ...
            string absoluteSceneLoadDataPath = Path.Combine(
                absoluteTempPath,
                OVRSceneLoader.sceneLoadDataName
            );
            string[] pushCommand =
            ...
            if (adbTool.RunCommand(pushCommand, null, out output, out error) == 0)
            {
                ...
                OculusBuildApp.SendBuildEvent(
                    "oculus_bundle_tool",
                    "transfer_bundle_time",
                    (DateTime.Now - transferStart).TotalSeconds.ToString()
                );
                return true;
            }
    ...
    private static bool TransferSceneBundles(
        OVRADBTool adbTool,
        string absoluteTempPath,
        string externalSceneCache
    ) {
        ...
        if (adbTool.RunCommand(pullManifestCommand, null, out output, out error) == 0)
        {
            ...
            Debug.Log(
                "[OVRBundleManager] - Scene bundle manifest file found. Decoding changes . . ."
            );

            // Load hashes from remote manifest
            AssetBundle remoteBundle = AssetBundle.LoadFromFile(
                Path.Combine(absoluteTempPath, BUNDLE_MANAGER_MASTER_BUNDLE)
            );
            ...
            AssetBundle localBundle = AssetBundle.LoadFromFile(
                BUNDLE_MANAGER_OUTPUT_PATH
                    + "\\"
                    + BUNDLE_MANAGER_MASTER_BUNDLE
                    + "\\"
            ...
            {
                OVRBundleTool.PrintError(
                    "<color=red>Failed to load local asset bundle manifest file.\n</color>"
                );
                return false;
            }
            ...
            if (localManifest != null)
            {
                ...
                string relativeSceneBundlesPath = Path.Combine(
                    BUNDLE_MANAGER_OUTPUT_PATH,
                    BUNDLE_MANAGER_MASTER_BUNDLE
                );
                ...
                foreach (string bundleName in assetBundles)
                {
                    ...
                    else
                    {
                        if (
                            ...
                            != localManifest.GetAssetBundleHash(bundleName)
                        ) {
                            bundlesToTransfer.Add(
                ...
                OVRBundleTool.PrintLog(
                    bundlesToTransfer.Count + " dirty bundle(s) will be transfered.\n"
                );
            }
        }
        else
        {
            if (output.Contains("does not exist") || output.Contains("No such file or directory"))
            {
                ...
                {
                    string absoluteSceneBundlePath = Path.Combine(
                        Path.Combine(Application.dataPath, ".."),
        ...
        {
            string absoluteBundlePath = Path.Combine(
                Path.Combine(Application.dataPath, ".."),
                bundle
            );
        ...
        Debug.Log(
            "[OVRBundleManager] - Transfer took "
                + (DateTime.Now - transferStart).TotalSeconds
                + " seconds."
        );
        ...
        if (bundlesToDelete.Count > 0)
        {
            ...
            OVRBundleTool.PrintLog(
                "Deleted " + bundlesToDelete.Count + " bundle(s) that were stale"
            );
        }
    ...
    public static bool LaunchApplication()
    {
        ...
        if (adbTool.isReady)
        {
            ...
            string playerActivityName =
                "\"" + appPackagename + "/com.unity3d.player.UnityPlayerActivity\"";
    ...
    {
        string[] resourceDirectories = Directory.GetDirectories(
            "Assets",
            "Resources",
            SearchOption.AllDirectories
        );
