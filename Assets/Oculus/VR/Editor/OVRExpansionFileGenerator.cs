    ...
    if (File.Exists(path + "Asset Bundles"))
    {
      ...
      try
      {
        ...
        UnityEngine.Debug.Log(
            "OBB expansion file "
                + expansionName
                + " has been successfully created at "
                + path
        );
    ...
    try
    {
      ...
      if (!string.IsNullOrEmpty(androidNamepsaceURI))
      {
        ...
        foreach (XmlElement e in nodeList)
        {
          ...
          {
            UnityEngine.Debug.Log(
                "Android manifest already has the proper permissions."
            );
            return;
          }
        }
        ...
        if (element != null)
        {
          ...
          UnityEngine.Debug.Log(
              "Successfully modified android manifest with external storage permission."
          );
          return;
        }
      }
