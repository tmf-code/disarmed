...
public class OVROverlayMeshGenerator : MonoBehaviour
{
  ...
  private void Update()
  {
    ...
    if (_Overlay)
    {
      ...
      Vector3 position = _CameraRoot
          ? (_Transform.position - _CameraRoot.position)
          : _Transform.position;
      Quaternion rotation = _Transform.rotation;
      Vector3 scale = _Transform.lossyScale;
      Rect destRectLeft = _Overlay.overrideTextureRectMatrix
          ? _Overlay.destRectLeft
          : new Rect(0, 0, 1, 1);
      Rect destRectRight = _Overlay.overrideTextureRectMatrix
          ? _Overlay.destRectRight
          : new Rect(0, 0, 1, 1);
      Rect srcRectLeft = _Overlay.overrideTextureRectMatrix
          ? _Overlay.srcRectLeft
          : new Rect(0, 0, 1, 1);
      ...
      )
      {
        UpdateMesh(
            ...
            rotation,
            scale,
            GetBoundingRect(destRectLeft, destRectRight)
        );
        _LastShape = shape;
        _LastPosition = position;
        _LastRotation = rotation;
  ...
  public static void BuildSphere(
      List<Vector3> verts,
      List<Vector2> uv,
      List<int> triangles,
      Vector3 position,
  ...
  {
    return BottomLeft[(int)face]
        + sideUV.x * RightVector[(int)face]
        + sideUV.y * UpVector[(int)face];
  }
  ...
  public static void BuildHemicylinder(
      List<Vector3> verts,
      List<Vector2> uv,
      List<int> triangles,
      Vector3 scale,
