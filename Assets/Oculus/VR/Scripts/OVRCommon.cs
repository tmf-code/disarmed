...
public static class OVRExtensions
{
  ...
  public static OVRTracker.Frustum ToFrustum(this OVRPlugin.Frustumf f)
  {
    return new OVRTracker.Frustum()
    {
      nearZ = f.zNear,
      farZ = f.zFar,
      fov = new Vector2() { x = Mathf.Rad2Deg * f.fovX, y = Mathf.Rad2Deg * f.fovY }
    };
  }
...
public static class OVRNodeStateProperties
{
  ...
  public static bool GetNodeStatePropertyVector3(
      Node nodeType,
      NodeStatePropertyType propertyType,
      OVRPlugin.Node ovrpNodeType,
  ...
  )
  {
    ...
    switch (propertyType)
    {
      case NodeStatePropertyType.Acceleration:
        ...
        {
          retVec = OVRPlugin.GetNodeAcceleration(ovrpNodeType, stepType)
      ...
      case NodeStatePropertyType.AngularAcceleration:
        if (OVRManager.loadedXRDevice == OVRManager.XRDevice.Oculus)
        {
          retVec = OVRPlugin.GetNodeAngularAcceleration(ovrpNodeType, stepType)
              .FromFlippedZVector3f();
          return true;
        }
        if (
      ...
      case NodeStatePropertyType.Velocity:
        if (OVRManager.loadedXRDevice == OVRManager.XRDevice.Oculus)
        {
          retVec = OVRPlugin.GetNodeVelocity(ovrpNodeType, stepType)
              .FromFlippedZVector3f();
          return true;
        }
        if (
            GetUnityXRNodeStateVector3(nodeType, NodeStatePropertyType.Velocity, out retVec)
        )
          return true;
        break;
      ...
      case NodeStatePropertyType.AngularVelocity:
        if (OVRManager.loadedXRDevice == OVRManager.XRDevice.Oculus)
        {
          retVec = OVRPlugin.GetNodeAngularVelocity(ovrpNodeType, stepType)
              .FromFlippedZVector3f();
          return true;
        }
        if (
      ...
      case NodeStatePropertyType.Position:
        ...
        if (
            GetUnityXRNodeStateVector3(nodeType, NodeStatePropertyType.Position, out retVec)
        )
          return true;
        break;
    }
  ...
  public static bool GetNodeStatePropertyQuaternion(
      Node nodeType,
      NodeStatePropertyType propertyType,
      OVRPlugin.Node ovrpNodeType,
      OVRPlugin.Step stepType,
  ...
  private static bool GetUnityXRNodeStateVector3(
      Node nodeType,
      NodeStatePropertyType propertyType,
      out Vector3 retVec
  )
  ...
  private static bool GetUnityXRNodeStateQuaternion(
      Node nodeType,
      NodeStatePropertyType propertyType,
      out Quaternion retQuat
  )
...
public struct OVRPose
{
  ...
  {
    get { return new OVRPose() { position = Vector3.zero, orientation = Quaternion.identity }; }
  }
