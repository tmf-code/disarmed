...
public class OVROverlay : MonoBehaviour
{
  ...
  /// </summary>
  [Tooltip(
      "The noDepthBufferTesting will stop layer's depth buffer compositing even if the engine has \"Shared Depth Buffer\" enabled. The layer's ordering will be used instead which is determined by it's composition depth and overlay/underlay type."
  )]
  ...
  /// </summary>
  public void OverrideOverlayTextureInfo(
      Texture srcTexture,
      IntPtr nativePtr,
  ...
  )
  {
    ...
    OVRPlugin.LayerDesc desc = OVRPlugin.CalculateLayerDesc(
        shape,
        layout,
        size,
        mipLevels,
  ...
  private bool CreateLayerTextures(bool useMipmaps, OVRPlugin.Sizei size, bool isHdr)
  {
    if (isExternalSurface)
    {
      if (externalSurfaceObject == System.IntPtr.Zero)
      {
        ...
        {
          Debug.LogFormat(
              "GetLayerAndroidSurfaceObject returns {0}",
    ...
    for (int eyeId = 0; eyeId < texturesPerStage; ++eyeId)
    {
      ...
      for (int stage = 0; stage < stageCount; ++stage)
      {
        ...
        if (
            currentOverlayShape != OverlayShape.Cubemap
            && currentOverlayShape != OverlayShape.OffcenterCubemap
        )
          sc = Texture2D.CreateExternalTexture(
              size.w,
              size.h,
              txFormat,
              useMipmaps,
  ...
  private void DestroyLayer()
  {
    if (layerIndex != -1)
    {
      ...
      OVRPlugin.EnqueueSubmitLayer(
          ...
          -1,
          0,
          OVRPose.identity.ToPosef_Legacy(),
          Vector3.one.ToVector3f(),
          layerIndex,
  ...
  public void UpdateTextureRectMatrix()
  {
    ...
    Rect srcRectLeftConverted = new Rect(
        srcRectLeft.x,
        isExternalSurface ^ invertTextureRects
            ? 1 - srcRectLeft.y - srcRectLeft.height
            : srcRectLeft.y,
        srcRectLeft.width,
        srcRectLeft.height
    );
    Rect srcRectRightConverted = new Rect(
        srcRectRight.x,
        isExternalSurface ^ invertTextureRects
            ? 1 - srcRectRight.y - srcRectRight.height
            : srcRectRight.y,
        srcRectRight.width,
        srcRectRight.height
    );
    Rect destRectLeftConverted = new Rect(
        destRectLeft.x,
        isExternalSurface ^ invertTextureRects
            ? 1 - destRectLeft.y - destRectLeft.height
            : destRectLeft.y,
        destRectLeft.width,
        destRectLeft.height
    );
    Rect destRectRightConverted = new Rect(
        destRectRight.x,
        isExternalSurface ^ invertTextureRects
            ? 1 - destRectRight.y - destRectRight.height
            : destRectRight.y,
        destRectRight.width,
        destRectRight.height
    );
    ...
    textureRectMatrix.leftScaleBias = new Vector4(
        leftWidthFactor,
        leftHeightFactor,
    ...
    textureRectMatrix.rightScaleBias = new Vector4(
        rightWidthFactor,
        rightHeightFactor,
  ...
  private bool LatchLayerTextures()
  {
    ...
    {
      if (
          textures[i] != layerTextures[i].appTexture
          || layerTextures[i].appTexturePtr == IntPtr.Zero
      )
      {
        if (textures[i] != null)
        {
          ...
          layerTextures[i].appTexturePtr =
              (texturePtrs[i] != IntPtr.Zero)
                  ? texturePtrs[i]
    ...
    {
      Debug.LogWarning(
          "Overlay shape " + currentOverlayShape + " is not supported on current platform"
      );
      return false;
    }
#endif
  ...
  private OVRPlugin.LayerDesc GetCurrentLayerDesc()
  {
    ...
    {
      if (
          texCube.format == TextureFormat.RGBAHalf
          || texCube.format == TextureFormat.RGBAFloat
      )
        newDesc.Format = OVRPlugin.EyeTextureFormat.R16G16B16A16_FP;
  ...
  private Rect GetBlitRect(int eyeId)
  {
    ...
    else
    {
      ...
      float maxX = Mathf.Max(
          srcRectLeft.x + srcRectLeft.width,
          srcRectRight.x + srcRectRight.width
      );
      float maxY = Mathf.Max(
          srcRectLeft.y + srcRectLeft.height,
          srcRectRight.y + srcRectRight.height
      );
  ...
  )
  {
    ...
    RenderTextureFormat rtFormat =
        (isHdr) ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.ARGB32;
    ...
    for (int eyeId = 0; eyeId < texturesPerStage; ++eyeId)
    {
      ...
      {
        bool dataIsLinear =
            isHdr || (QualitySettings.activeColorSpace == ColorSpace.Linear);
        ...
        bool bypassBlit =
            !linearToSRGB && !premultiplyAlpha && rt != null && rt.format == rtFormat;
        ...
        if (!bypassBlit)
        {
          ...
          blitMat.SetInt(
              "_flip",
              OVRPlugin.nativeXrApi == OVRPlugin.XrApi.OpenXR ? 1 : 0
          );
        }
  ...
  )
  {
    ...
    bool isOverlayVisible = OVRPlugin.EnqueueSubmitLayer(
        overlay,
        headLocked,
        ...
        layerId,
        frameIndex,
        pose.flipZ().ToPosef_Legacy(),
        scale.ToVector3f(),
        layerIndex,
        (OVRPlugin.OverlayShape)currentOverlayShape,
  ...
  private void SetupEditorPreview()
  {
    ...
    if (previewInEditor && previewObject == null)
    {
      ...
      OVROverlayMeshGenerator generator =
          previewObject.AddComponent<OVROverlayMeshGenerator>();
  ...
  void InitOVROverlay()
  {
    ...
    if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
    {
      ...
      if (overlay != null)
      {
        OVR.OpenVR.EVROverlayError error = overlay.CreateOverlay(
            OpenVROverlayKey + transform.name,
            gameObject.name,
            ref OpenVROverlayHandle
        );
  ...
  bool ComputeSubmit(ref OVRPose pose, ref Vector3 scale, ref bool overlay, ref bool headLocked)
  {
    ...
    pose =
        (headLocked)
            ? transform.ToHeadSpacePose(headCamera)
            : transform.ToTrackingSpacePose(headCamera);
    ...
    if (currentOverlayShape == OverlayShape.OffcenterCubemap)
    {
      ...
      {
        Debug.LogWarning(
            "Your cube map center offset's magnitude is greater than 1, which will cause some cube map pixel always invisible ."
        );
        return false;
      }
    }
    ...
    if (
        OVRPlugin.nativeXrApi != OVRPlugin.XrApi.OpenXR
        && currentOverlayShape == OverlayShape.Cylinder
    )
    {
      ...
      {
        Debug.LogWarning(
            "Cylinder overlay's arc angle has to be below 180 degree, current arc angle is "
                + arcAngle
                + " degree."
        );
        return false;
    ...
    if (
        OVRPlugin.nativeXrApi == OVRPlugin.XrApi.OpenXR
        && currentOverlayShape == OverlayShape.Fisheye
    )
    {
  ...
  void OpenVROverlayUpdate(Vector3 scale, OVRPose pose)
  {
    ...
    if (overlayTex != null)
    {
      ...
      if (
          error == OVR.OpenVR.EVROverlayError.InvalidHandle
          || error == OVR.OpenVR.EVROverlayError.UnknownOverlay
      )
      {
        if (
            overlayRef.FindOverlay(
                OpenVROverlayKey + transform.name,
                ref OpenVROverlayHandle
      ...
      overlayRef.SetOverlayTransformAbsolute(
          OpenVROverlayHandle,
          OVR.OpenVR.ETrackingUniverseOrigin.TrackingUniverseStanding,
          ref pose34
      );
    }
  }
  ...
  void LateUpdate()
  {
    ...
    bool isOverlayVisible = SubmitLayer(
        overlay,
        headLocked,
        noDepthBufferTesting,
        pose,
