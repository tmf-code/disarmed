...
public static class OVRHaptics
{
  ...
  public static class Config
  {
    ...
    {
      OVRPlugin.HapticsDesc desc = OVRPlugin.GetControllerHapticsDesc(
          (uint)OVRPlugin.Controller.RTouch
      );
  ...
  private class OVRHapticsOutput
  {
    ...
    private OVRNativeBuffer m_nativeBuffer = new OVRNativeBuffer(
        OVRHaptics.Config.MaximumBufferSamplesCount * OVRHaptics.Config.SampleSizeInBytes
    );
    ...
    public void Process()
    {
      ...
      if (m_prevSamplesQueued > 0)
      {
        int expectedSamples =
            m_prevSamplesQueued
            - (int)(elapsedTime * OVRHaptics.Config.SampleRateHz + 0.5f);
      ...
      if (m_lowLatencyMode)
      {
        ...
        int lowLatencySamplesCount =
            OVRHaptics.Config.MinimumSafeSamplesQueued + samplesNeededPerFrame;
      ...
      while (acquiredSamplesCount < desiredSamplesCount && clipIndex < m_pendingClips.Count)
      {
        ...
        int remainingSamplesInClip =
            m_pendingClips[clipIndex].Clip.Count - m_pendingClips[clipIndex].ReadCount;
        ...
        if (numSamplesToCopy > 0)
        {
          ...
          int srcOffset =
              m_pendingClips[clipIndex].ReadCount * OVRHaptics.Config.SampleSizeInBytes;
          Marshal.Copy(
              m_pendingClips[clipIndex].Clip.Samples,
              srcOffset,
              m_nativeBuffer.GetPointer(dstOffset),
              numBytes
          );
      ...
      {
        int desiredPadding =
            desiredSamplesCount - (hapticsState.SamplesQueued + acquiredSamplesCount);
        if (
            desiredPadding
            < (OVRHaptics.Config.MinimumBufferSamplesCount - acquiredSamplesCount)
        )
          desiredPadding = (
              OVRHaptics.Config.MinimumBufferSamplesCount - acquiredSamplesCount
          );
    ...
    public void Mix(OVRHapticsClip clip)
    {
      ...
      while (numSamplesRemaining > 0 && numClipsToMix < m_pendingClips.Count)
      {
        int numSamplesRemainingInClip =
            m_pendingClips[numClipsToMix].Clip.Count
            - m_pendingClips[numClipsToMix].ReadCount;
      ...
      if (numClipsToMix > 0)
      {
        ...
        for (int i = 0; i < numClipsToMix; i++)
        {
          ...
          for (
              int bReadCount = m_pendingClips[i].ReadCount;
              bReadCount < b.Count;
              bReadCount++
          )
          {
            if (OVRHaptics.Config.SampleSizeInBytes == 1)
            {
              ...
              if ((aReadCount < a.Count) && (bReadCount < b.Count))
              {
                sample = (byte)(
                    Mathf.Clamp(
                        a.Samples[aReadCount] + b.Samples[bReadCount],
                        0,
                        System.Byte.MaxValue
                    )
                ); // TODO support multi-byte samples
                aReadCount++;
              }
