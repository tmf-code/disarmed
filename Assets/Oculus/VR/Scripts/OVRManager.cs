...
#pragma warning restore
  ...
  public delegate GameObject InstantiateMrcCameraDelegate(
      GameObject mainCameraGameObject,
      MrcCameraType cameraType
  );
  ...
  [
      HideInInspector,
      Tooltip(
          "Specify if Insight Passthrough should be enabled. Passthrough overlays/underlays can only be used if passthrough is enabled."
      )
  ]
  ...
  /// and have no effect.
  /// </summary>
  public static void SetColorScaleAndOffset(
      Vector4 colorScale,
      Vector4 colorOffset,
  ...
  private static Vector3 OpenVRTouchPositionOffsetRight = new Vector3(
      -0.0075f,
      -0.005f,
      -0.0525f
  );
  ...
  public static OVRPose GetOpenVRControllerOffset(Node hand)
  {
    ...
    if ((hand == Node.LeftHand || hand == Node.RightHand) && loadedXRDevice == XRDevice.OpenVR)
    {
      ...
      if (
          OVRInput.openVRControllerDetails[index].controllerType
          == OVRInput.OpenVRController.OculusTouch
      )
      {
        ...
        poseOffset.orientation = Quaternion.Euler(
            offsetOrientation.x,
            offsetOrientation.y,
            offsetOrientation.z
        );
  ...
  /// </summary>
  [Tooltip(
      "If true, rendered controller latency is reduced by several ms, as the left/right controllers will have their positions updated right before rendering."
  )]
  ...
  private void InitOVRManager()
  {
    ...
    // Debug.LogFormat(
    //     "SystemHeadset {0}, API {1}",
    //     systemHeadsetType.ToString(),
    //     xrApi.ToString()
    // );
    ...
    if (!supportedTypes.Contains(SystemInfo.graphicsDeviceType.ToString()))
      Debug.LogWarning(
          "VR rendering requires one of the following device types: ("
              + supportedTypes
    ...
    if (
        currPlatform == RuntimePlatform.Android
        ||
    ...
    // Debug.LogFormat(
    //     "Current display frequency {0}, available frequencies [{1}]",
    //     display.displayFrequency,
    //     string.Join(
    //         ", ",
    //         display.displayFrequenciesAvailable.Select(f => f.ToString()).ToArray()
    //     )
    // );
  ...
  void SetCurrentXRDevice()
  {
    ...
    XRDisplaySubsystemDescriptor currentDisplaySubsystemDescriptor =
    ...
    if (OVRPlugin.initialized)
    {
      loadedXRDevice = XRDevice.Oculus;
        ...
        currentDisplaySubsystem != null
        && currentDisplaySubsystemDescriptor != null
  ...
  public static void StaticInitializeMixedRealityCapture(
  ...
  )
  {
    if (!staticMixedRealityCaptureInitialized)
    {
      ...
      staticMrcSettings.ReadFrom(configuration);
            ...
            Debug.Log(
                mediaInitialized ? "OVRPlugin.Media initialized" : "OVRPlugin.Media not initialized"
            );
            if (mediaInitialized)
            {
                ...
                OVRPlugin.Media.SetMrcInputVideoBufferType(
                    OVRPlugin.Media.InputVideoBufferType.TextureHandle
                );
                Debug.LogFormat(
                    "[MRC] Active InputVideoBufferType:{0}",
                    OVRPlugin.Media.GetMrcInputVideoBufferType()
                );
                if (configuration.mrcActivationMode == MrcActivationMode.Automatic)
                {
                    OVRPlugin.Media.SetMrcActivationMode(
                        OVRPlugin.Media.MrcActivationMode.Automatic
                    );
                ...
                {
                    OVRPlugin.Media.SetMrcActivationMode(
                        OVRPlugin.Media.MrcActivationMode.Disabled
                    );
  ...
  public static void StaticUpdateMixedRealityCapture(
      OVRMixedRealityCaptureConfiguration configuration,
    ...
    if (!staticMixedRealityCaptureInitialized)
    {
      return;
        ...
        configuration.enableMixedReality =
            OVRPlugin.Media.GetInitialized() && OVRPlugin.Media.IsMrcActivated();
  ...
  public static void StaticShutdownMixedRealityCapture(
      OVRMixedRealityCaptureConfiguration configuration
  )
  {
    if (staticMixedRealityCaptureInitialized)
