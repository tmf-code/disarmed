...
public class OVRDirectComposition : OVRCameraComposition
{
  ...
  )
  {
    ...
    {
      Debug.LogWarning(
          "[OVRDirectComposition] RefreshCameraObjects(): Unable to open camera device "
              + cameraDevice
      );
      return;
    }
    ...
    {
      Debug.LogFormat(
          "[OVRDirectComposition] Camera refreshed. Rebind camera to {0}",
      ...
      if (configuration.instantiateMixedRealityCameraGameObject != null)
      {
        directCompositionCameraGameObject =
            configuration.instantiateMixedRealityCameraGameObject(
      ...
      Debug.Log(
          "DirectComposition activated : useDynamicLighting "
  ...
  )
  {
    ...
    {
      Debug.LogWarning(
          "HandPoseStateLatency is invalid. Expect a value between 0.0 to 0.5, get "
    ...
    if (hasCameraDeviceOpened)
    {
      if (
          ...
          || boundaryMeshMaskTexture.width != Screen.width
          || boundaryMeshMaskTexture.height != Screen.height
      )
      {
        boundaryMeshMaskTexture = new RenderTexture(
            Screen.width,
            Screen.height,
            0,
