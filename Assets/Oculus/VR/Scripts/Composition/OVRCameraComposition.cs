...
public abstract class OVRCameraComposition : OVRComposition
{
  ...
  private OVRManager.VirtualGreenScreenType boundaryMeshType =
      OVRManager.VirtualGreenScreenType.Off;
  ...
  ) : base(parentObject, mainCamera, configuration)
  {
    ...
    Debug.Assert(
        !OVRPlugin.IsCameraDeviceAvailable(cameraDevice)
            || !OVRPlugin.HasCameraDeviceOpened(cameraDevice)
    );
    ...
    {
      Debug.LogWarning(
          "The camera device doesn't support depth. The result of dynamic lighting might not be correct"
      );
    }
    ...
    if (OVRPlugin.IsCameraDeviceAvailable(cameraDevice))
    {
      ...
      if (
          OVRPlugin.GetExternalCameraCount() > 0
          && OVRPlugin.GetMixedRealityCameraInfo(0, out extrinsics, out intrinsics)
      )
      {
        OVRPlugin.SetCameraDevicePreferredColorFrameSize(
            cameraDevice,
            intrinsics.ImageSensorPixelResolution.w,
            intrinsics.ImageSensorPixelResolution.h
        );
      }

      if (configuration.useDynamicLighting)
      {
        OVRPlugin.SetCameraDeviceDepthSensingMode(
            cameraDevice,
            OVRPlugin.CameraDeviceDepthSensingMode.Fill
        );
        OVRPlugin.CameraDeviceDepthQuality quality =
            OVRPlugin.CameraDeviceDepthQuality.Medium;
  ...
  protected void RefreshCameraFramePlaneObject(
      GameObject parentObject,
  ...
  )
  {
    ...
    cameraFramePlaneObject.transform.parent = cameraInTrackingSpace
        ? cameraRig.trackingSpace
        : parentObject.transform;
    cameraFramePlaneObject.GetComponent<Collider>().enabled = false;
    cameraFramePlaneObject.GetComponent<MeshRenderer>().shadowCastingMode =
        UnityEngine.Rendering.ShadowCastingMode.Off;
    Material cameraFrameMaterial = new Material(
        Shader.Find(
            configuration.useDynamicLighting
                ? "Oculus/OVRMRCameraFrameLit"
                : "Oculus/OVRMRCameraFrame"
        )
    );
    ...
    cameraFrameCompositionManager =
        mixedRealityCamera.gameObject.AddComponent<OVRCameraFrameCompositionManager>();
  ...
  )
  {
    ...
    if (!hasError)
    {
      ...
      cameraFramePlaneObject.transform.position =
          mixedRealityCamera.transform.position
          + mixedRealityCamera.transform.forward * distance;
      ...
      cameraFramePlaneObject.transform.localScale = new Vector3(
          distance * mixedRealityCamera.aspect * tanFov * 2.0f,
          distance * tanFov * 2.0f,
          1.0f
      );
      ...
      cameraFrameMaterial.SetVector(
          "_FlipParams",
          new Vector4(
              (configuration.flipCameraFrameHorizontally ? 1.0f : 0.0f),
              (configuration.flipCameraFrameVertically ? 1.0f : 0.0f),
              0.0f,
              0.0f
          )
      );
      ...
      cameraFrameMaterial.SetFloat(
          "_ChromaKeySmoothRange",
          configuration.chromaKeySmoothRange
      );
      ...
      cameraFrameMaterial.SetVector(
          ...
          new Vector4(
              colorTexture.width,
              colorTexture.height,
              1.0f / colorTexture.width,
              1.0f / colorTexture.height
          )
      );
      cameraFrameMaterial.SetVector(
          "_TextureWorldSize",
          new Vector4(worldWidth, worldHeight, 0, 0)
      );
      cameraFrameMaterial.SetFloat(
          "_SmoothFactor",
          configuration.dynamicLightingSmoothFactor
      );
      cameraFrameMaterial.SetFloat(
          "_DepthVariationClamp",
          configuration.dynamicLightingDepthVariationClampingValue
      );
      cameraFrameMaterial.SetFloat("_CullingDistance", cullingDistance);
      if (
          configuration.virtualGreenScreenType == OVRManager.VirtualGreenScreenType.Off
          || boundaryMesh == null
          || boundaryMeshMaskTexture == null
      )
      {
      ...
      else
      {
        if (cameraRig == null)
        {
          ...
          {
            Debug.LogWarning(
                "Could not find the OVRCameraRig/CenterEyeAnchor object. Please check if the OVRCameraRig has been setup properly. The virtual green screen has been temporarily disabled"
            );
        ...
        else
        {
          ...
          {
            Debug.Log(
                "OVRCameraRig/CenterEyeAnchor object found. Virtual green screen is activated"
            );
  ...
  protected void RefreshBoundaryMesh(
      Camera camera,
      OVRMixedRealityCaptureConfiguration configuration,
      out float cullingDistance
  )
  {
    float depthTolerance = configuration.virtualGreenScreenApplyDepthCulling
        ? configuration.virtualGreenScreenDepthTolerance
        : float.PositiveInfinity;
    cullingDistance =
        OVRCompositionUtil.GetMaximumBoundaryDistance(
            camera,
            OVRCompositionUtil.ToBoundaryType(configuration.virtualGreenScreenType)
        ) + depthTolerance;
    if (
        boundaryMesh == null
    ...
    )
    {
      boundaryMeshTopY = configuration.virtualGreenScreenTopY;
      boundaryMeshBottomY = configuration.virtualGreenScreenBottomY;
      boundaryMesh = OVRCompositionUtil.BuildBoundaryMesh(
          OVRCompositionUtil.ToBoundaryType(configuration.virtualGreenScreenType),
          boundaryMeshTopY,
          boundaryMeshBottomY
      );
        ...
        for (int i = 0; i < whiteMaterial.passCount; ++i)
        {
          if (whiteMaterial.SetPass(i))
          {
            Graphics.DrawMeshNow(
                composition.boundaryMesh,
                composition.cameraRig.ComputeTrackReferenceMatrix()
            );
          }
        }
