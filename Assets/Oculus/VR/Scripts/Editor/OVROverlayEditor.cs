...
public class OVROverlayEditor : Editor
{
      ...
      if (_SrcRectMaterial == null)
      {
        ...
        if (shaders.Length > 0)
        {
          Shader shader = (Shader)AssetDatabase.LoadAssetAtPath(
              AssetDatabase.GUIDToAssetPath(shaders[0]),
              typeof(Shader)
          );
  ...
  private void Awake()
  {
    ...
    List<OVROverlay.OverlayShape> selectableShapesValueList =
        new List<OVROverlay.OverlayShape>();
  ...
  public override void OnInspectorGUI()
  {
    ...
    overlay.currentOverlayType = (OVROverlay.OverlayType)EditorGUILayout.EnumPopup(
        new GUIContent(
            "Current Overlay Type",
            "Whether this overlay should layer behind the scene or in front of it"
        ),
        overlay.currentOverlayType
    );
    overlay.compositionDepth = EditorGUILayout.IntField(
        new GUIContent(
            "Composition Depth",
            "Depth value used to sort OVROverlays in the scene, smaller value appears in front"
        ),
        overlay.compositionDepth
    );
    overlay.noDepthBufferTesting = EditorGUILayout.Toggle(
        new GUIContent(
            "No Depth Buffer Testing",
            "The noDepthBufferTesting will stop layer's depth buffer compositing even if the engine has \"Shared Depth Buffer\" enabled"
        ),
        overlay.noDepthBufferTesting
    );
    EditorGUILayout.Space();

    EditorGUILayout.LabelField(
        new GUIContent("Overlay Shape", "The shape of this overlay"),
    ...
    currentShapeIndex = EditorGUILayout.Popup(
        new GUIContent("Overlay Shape", "The shape of this overlay"),
        currentShapeIndex,
        selectableShapeNames
    );
    ...
    EditorGUILayout.LabelField("Textures", EditorStyles.boldLabel);
        ...
        overlay.isExternalSurface = EditorGUILayout.Toggle(
            new GUIContent(
                "Is External Surface",
                "On Android, retrieve an Android Surface object to render to (e.g., video playback)"
            ),
            overlay.isExternalSurface
        );

        if (lastIsExternalSurface)
        {
            overlay.externalSurfaceWidth = EditorGUILayout.IntField(
                "External Surface Width",
                overlay.externalSurfaceWidth
            );
            overlay.externalSurfaceHeight = EditorGUILayout.IntField(
                "External Surface Height",
                overlay.externalSurfaceHeight
            );
            overlay.isProtectedContent = EditorGUILayout.Toggle(
                new GUIContent(
                    "Is Protected Content",
                    "The external surface has L1 widevine protection."
                ),
                overlay.isProtectedContent
            );
        }
        else
#endif
    {
      ...
      EditorGUI.LabelField(
          new Rect(
              labelControlRect.x,
              labelControlRect.y,
              labelControlRect.width / 2,
              labelControlRect.height
          ),
          new GUIContent("Left Texture", "Texture used for the left eye")
      );
      EditorGUI.LabelField(
          new Rect(
              labelControlRect.x + labelControlRect.width / 2,
              labelControlRect.y,
              labelControlRect.width / 2,
              labelControlRect.height
          ),
          new GUIContent("Right Texture", "Texture used for the right eye")
      );

      var textureControlRect = EditorGUILayout.GetControlRect(GUILayout.Height(64));

      overlay.textures[0] = (Texture)EditorGUI.ObjectField(
          new Rect(textureControlRect.x, textureControlRect.y, 64, textureControlRect.height),
          overlay.textures[0],
          typeof(Texture),
          true
      );
      Texture right = (Texture)EditorGUI.ObjectField(
          new Rect(
              textureControlRect.x + textureControlRect.width / 2,
              textureControlRect.y,
              64,
              textureControlRect.height
          ),
          overlay.textures[1] != null ? overlay.textures[1] : overlay.textures[0],
          typeof(Texture),
          true
      );
      ...
      overlay.isDynamic = EditorGUILayout.Toggle(
          new GUIContent(
              "Dynamic Texture",
              "This texture will be updated dynamically at runtime (e.g., Video)"
          ),
          overlay.isDynamic
      );
#if !UNITY_ANDROID
      overlay.isProtectedContent = EditorGUILayout.Toggle(
          new GUIContent(
              "Is Protected Content",
              "The texture has copy protection, e.g., HDCP"
          ),
          overlay.isProtectedContent
      );
#endif
    ...
    if (
        overlay.currentOverlayShape == OVROverlay.OverlayShape.Cylinder
    ...
    )
    {
      ...
      overlay.overrideTextureRectMatrix = !EditorGUILayout.Toggle(
          new GUIContent(
              "Use Default Rects",
              overlay.textures[1] == null
                  ? "If you need to use a single texture as a stereo image, uncheck this box"
                  : "Uncheck this box if you need to clip you textures or layer"
          ),
      ...
      {
        sourceRectsVisible = EditorGUILayout.Foldout(
            sourceRectsVisible,
            new GUIContent(
                "Source Rects",
        ...
        if (sourceRectsVisible)
        {
          ...
          if (mat != null)
          {
            ...
            Vector4 srcLeft = new Vector4(
                Mathf.Max(0.0f, overlay.srcRectLeft.x),
            ...
            Vector4 srcRight = new Vector4(
                Mathf.Max(0.0f, overlay.srcRectRight.x),
            ...
            var drawRectCentered = new Rect(
                drawRect.x + drawRect.width / 2 - 128 - 4,
                drawRect.y,
                256 + 8,
                drawRect.height
            );
            EditorGUI.DrawPreviewTexture(
                drawRectCentered,
                overlay.textures[0] ?? Texture2D.blackTexture,
                mat
            );

            var drawRectInset = new Rect(
                drawRectCentered.x + 4,
                drawRectCentered.y + 4,
                drawRectCentered.width - 8,
                drawRectCentered.height - 8
            );
            UpdateRectDragging(
                drawRectInset,
                drawRectInset,
                TextureRect.SrcLeft,
                TextureRect.SrcRight,
                overlay.invertTextureRects,
                ref overlay.srcRectLeft,
                ref overlay.srcRectRight
            );
            CreateCursorRects(
                drawRectInset,
                overlay.srcRectLeft,
          ...
          overlay.srcRectLeft = Clamp01(
              EditorGUI.RectField(
                  new Rect(
                      rectControlRect.x,
                      rectControlRect.y,
                      rectControlRect.width / 2 - 20,
                      rectControlRect.height
                  ),
                  overlay.srcRectLeft
              )
          );
          overlay.srcRectRight = Clamp01(
              EditorGUI.RectField(
                  new Rect(
                      rectControlRect.x + rectControlRect.width / 2,
                      rectControlRect.y,
                      rectControlRect.width / 2 - 20,
                      rectControlRect.height
                  ),
          ...
          if (overlay.textures[1] != null)
          {
            if (
                GUILayout.Button(
                    new GUIContent("Reset To Default", "Reset Source Rects to default")
                )
            )
            {
          ...
          else
          {
            if (
                GUILayout.Button(
                    new GUIContent(
                        "Monoscopic",
                        "Display the full Texture in both eyes"
                    )
                )
            )
            {
        ...
        destRectsVisible = EditorGUILayout.Foldout(
            destRectsVisible,
            new GUIContent(
                "Destination Rects",
        ...
        if (destRectsVisible)
        {
          ...
          if (mat != null)
          {
            ...
            Vector4 destLeft = new Vector4(
                Mathf.Max(0.0f, overlay.destRectLeft.x),
            ...
            Vector4 destRight = new Vector4(
                Mathf.Max(0.0f, overlay.destRectRight.x),
            ...
            var drawRectCentered = new Rect(
                drawRect.x + drawRect.width / 2 - 128 - 16 - 4,
                drawRect.y,
                256 + 32 + 8,
                drawRect.height
            );
            ...
            var drawRectInsetLeft = new Rect(
                drawRectCentered.x + 4,
                drawRectCentered.y + 4,
                drawRectCentered.width / 2 - 20,
                drawRectCentered.height - 8
            );
            var drawRectInsetRight = new Rect(
                drawRectCentered.x + drawRectCentered.width / 2 + 16,
                drawRectCentered.y + 4,
                drawRectCentered.width / 2 - 20,
                drawRectCentered.height - 8
            );
            UpdateRectDragging(
                drawRectInsetLeft,
                drawRectInsetRight,
                TextureRect.DestLeft,
                TextureRect.DestRight,
                overlay.invertTextureRects,
                ref overlay.destRectLeft,
                ref overlay.destRectRight
            );

            CreateCursorRects(
                drawRectInsetLeft,
                overlay.destRectLeft,
                overlay.invertTextureRects
            );
            CreateCursorRects(
                drawRectInsetRight,
                overlay.destRectRight,
                overlay.invertTextureRects
            );
          }
          ...
          if (overlay.currentOverlayShape == OVROverlay.OverlayShape.Equirect)
          {
            ...
            if (
                GUILayout.Button(
                    new GUIContent("360 Video", "Display the full 360 layer")
                )
            )
            {
            ...
            if (
                GUILayout.Button(
                    new GUIContent("180 Video", "Display the front 180 layer")
                )
            )
            {
        ...
        overlay.invertTextureRects = EditorGUILayout.Toggle(
            new GUIContent(
                "Invert Rect Coordinates",
    ...
    EditorGUILayout.Space();
    overlay.overridePerLayerColorScaleAndOffset = EditorGUILayout.Toggle(
        new GUIContent(
        ...
        overlay.overridePerLayerColorScaleAndOffset
    );
    if (overlay.overridePerLayerColorScaleAndOffset)
    {
      Vector4 colorScale = EditorGUILayout.Vector4Field(
          new GUIContent(
              "Color Scale",
              "Scale that the color values for this overlay will be multiplied by."
          ),
          overlay.colorScale
      );
      Vector4 colorOffset = EditorGUILayout.Vector4Field(
          new GUIContent(
              "Color Offset",
              "Offset that the color values for this overlay will be added to."
          ),
          overlay.colorOffset
      );
    ...
    overlay.previewInEditor = EditorGUILayout.Toggle(
        new GUIContent(
            "Preview in Editor (Experimental)",
            "Preview the overlay in the editor using a mesh renderer."
        ),
        overlay.previewInEditor
    );
    ...
    else if (overlay.textures[0] != null)
    {
      if (IsUnitRect(overlay.srcRectLeft) && IsUnitRect(overlay.srcRectRight))
      {
        return StereoType.Mono;
          ...
          IsRect(overlay.srcRectLeft, 0, 0, 0.5f, 1f)
          && IsRect(overlay.srcRectRight, 0.5f, 0, 0.5f, 1f)
      )
      {
        return StereoType.StereoLeftRight;
          ...
          overlay.invertTextureRects
          && IsRect(overlay.srcRectLeft, 0, 0.0f, 1f, 0.5f)
      ...
      )
      {
        return StereoType.StereoTopBottom;
          ...
          !overlay.invertTextureRects
          && IsRect(overlay.srcRectLeft, 0, 0.5f, 1f, 0.5f)
  ...
  private void SetRectsByVideoType(
      OVROverlay overlay,
  ...
  )
  {
    ...
    leftPos = new Vector2(
        drawRect.x + selectRect.x * drawRect.width,
        drawRect.y + (1 - selectRect.y - selectRect.height / 2) * drawRect.height
    );
    rightPos = new Vector2(
        drawRect.x + (selectRect.x + selectRect.width) * drawRect.width,
        drawRect.y + (1 - selectRect.y - selectRect.height / 2) * drawRect.height
    );
    topPos = new Vector2(
        drawRect.x + (selectRect.x + selectRect.width / 2) * drawRect.width,
  ...
  private void CreateCursorRects(Rect drawRect, Rect selectRect, bool invertY)
  {
    ...
    EditorGUIUtility.AddCursorRect(
        new Rect(leftPos - 5 * Vector2.one, 10 * Vector2.one),
        MouseCursor.ResizeHorizontal
    );
    EditorGUIUtility.AddCursorRect(
        new Rect(rightPos - 5 * Vector2.one, 10 * Vector2.one),
        MouseCursor.ResizeHorizontal
    );
    EditorGUIUtility.AddCursorRect(
        new Rect(topPos - 5 * Vector2.one, 10 * Vector2.one),
        MouseCursor.ResizeVertical
    );
    EditorGUIUtility.AddCursorRect(
        new Rect(bottomPos - 5 * Vector2.one, 10 * Vector2.one),
        MouseCursor.ResizeVertical
    );
  }

  private bool IsOverRectControls(
      Rect drawRect,
      Vector2 mousePos,
      Rect selectRect,
      bool invertY,
  ...
  private void UpdateRectDragging(
      Rect drawingRectLeft,
      Rect drawingRectRight,
      TextureRect rectLeftType,
      TextureRect rectRightType,
      bool invertY,
      ref Rect rectLeft,
      ref Rect rectRight
  )
  ...
  )
  {
    ...
    float x = Mathf.Clamp01(
        Mathf.Round(((mousePos.x - drawingRect.x) / drawingRect.width) * 32) / 32.0f
    );
    float y = Mathf.Clamp01(
        Mathf.Round(((mousePos.y - drawingRect.y) / drawingRect.height) * 32) / 32.0f
    );
    if (!invertY)
    {
      y = 1 - y;
