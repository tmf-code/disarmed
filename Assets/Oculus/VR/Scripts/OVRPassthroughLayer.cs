...
public class OVRPassthroughLayer : MonoBehaviour
{
  ...
  public void RemoveSurfaceGeometry(GameObject obj)
  {
    ...
    {
      if (
          OVRPlugin.DestroyInsightPassthroughGeometryInstance(
              passthroughMeshInstance.instanceHandle
  ...
  {
    return surfaceGameObjects.ContainsKey(obj)
        || deferredSurfaceGameObjects.Exists(x => x.gameObject == obj);
  }
    ...
    for (int i = 0; i < deferredSurfaceGameObjects.Count; ++i)
    {
      ...
      else
      {
        ...
        if (
            CreateAndAddMesh(
                entry.gameObject,
                entry.worldToTrackingSpace,
                out meshHandle,
                out instanceHandle
            )
        )
        {
          surfaceGameObjects.Add(
              entry.gameObject,
              new PassthroughMeshInstance
              {
  ...
  )
  {
    ...
    if (
        !OVRPlugin.AddInsightPassthroughSurfaceGeometry(
            passthroughOverlay.layerId,
            meshHandle,
  ...
  private void UpdateSurfaceGeometryTransforms()
  {
    ...
    foreach (KeyValuePair<GameObject, PassthroughMeshInstance> el in surfaceGameObjects)
    {
      if (el.Value.updateTransform && el.Value.instanceHandle != 0)
      {
        Matrix4x4 T_worldInsight_model = GetTransformMatrixForPassthroughSurfaceObject(
            el.Key,
            el.Value.worldToTrackingSpace
        );
        if (
            !OVRPlugin.UpdateInsightPassthroughGeometryTransform(
    ...
    )
    {
      ...
      for (int i = 0; i < 256; i++)
      {
        ...
        if (colorMapEditorPosterize > 0.0f)
        {
          ...
          double posterize =
              (Math.Pow(posterizationBase, colorMapEditorPosterize) - 1.0)
              / (posterizationBase - 1.0);
  ...
  private Dictionary<GameObject, PassthroughMeshInstance> surfaceGameObjects = new Dictionary<
      GameObject,
  ...
  void LateUpdate()
  {
    ...
    if (styleDirty)
    {
      ...
      style.Flags =
          OVRPlugin.InsightPassthroughStyleFlags.HasTextureOpacityFactor
  ...
  void OnEnable()
  {
    ...
    passthroughOverlay.currentOverlayShape =
        projectionSurfaceType == ProjectionSurfaceType.UserDefined
