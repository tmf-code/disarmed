...
public class OVRCameraRig : MonoBehaviour
{
  ...
  protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
  {
    ...
    if (updateHandAnchors)
    {
      ...
      {
        leftHandAnchor.localPosition = OVRInput.GetLocalControllerPosition(
            OVRInput.Controller.LTouch
        );
        rightHandAnchor.localPosition = OVRInput.GetLocalControllerPosition(
            OVRInput.Controller.RTouch
        );
        leftHandAnchor.localRotation = OVRInput.GetLocalControllerRotation(
            OVRInput.Controller.LTouch
        );
        rightHandAnchor.localRotation = OVRInput.GetLocalControllerRotation(
            OVRInput.Controller.RTouch
        );
      }
  ...
  public virtual void EnsureGameObjectIntegrity()
  {
    ...
    else
    {
      ...
      if (
          ...
          || _rightEyeCamera.enabled
              == !(usePerEyeCameras && (!monoscopic || OVRPlugin.EyeTextureArrayEnabled))
      )
      {
        _skipUpdate = true;
      }
  ...
  public virtual Matrix4x4 ComputeTrackReferenceMatrix()
  {
    ...
    Matrix4x4 invHeadMatrix = Matrix4x4.TRS(
        invHeadPose.position,
        invHeadPose.orientation,
        Vector3.one
    );
