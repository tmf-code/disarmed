...
public static class OVRInput
{
  ...
  private static System.Version _pluginSupportsActiveControllerMinVersion = new System.Version(
      1,
      9,
      0
  );
...
#endif
        _pluginSupportsActiveController =
            isSupportedPlatform
            && (OVRPlugin.version >= _pluginSupportsActiveControllerMinVersion);
  ...
  public static void Update()
  {
    ...
    for (int i = 0; i < controllers.Count; i++)
    {
      ...
      if ((connectedControllerTypes & controller.controllerType) != 0)
      {
        ...
        if (
            Get(rawButtonMask, controller.controllerType)
    ...
    if (
        (activeControllerType == Controller.LTouch)
    ...
    if (
        (activeControllerType == Controller.LHand) || (activeControllerType == Controller.RHand)
    )
    {
    ...
    )
    {
      ...
      if (
          activeControllerType == Controller.None
          && ((localActiveController & Controller.Hands) != 0)
      )
      {
  ...
  public static void FixedUpdate()
  {
    if (OVRPlugin.nativeXrApi != OVRPlugin.XrApi.OpenXR)
    {
      ...
      double predictionSeconds =
          (double)fixedUpdateCount * Time.fixedDeltaTime / Mathf.Max(Time.timeScale, 1e-6f);
      fixedUpdateCount++;
    ...
    switch (controllerType)
    {
      ...
      case Controller.LHand:
        if (OVRManager.loadedXRDevice == OVRManager.XRDevice.Oculus)
          return OVRPlugin.GetNodePose(OVRPlugin.Node.HandLeft, stepType)
              .ToOVRPose().position;
        else if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
      ...
      case Controller.RHand:
        if (OVRManager.loadedXRDevice == OVRManager.XRDevice.Oculus)
          return OVRPlugin.GetNodePose(OVRPlugin.Node.HandRight, stepType)
              .ToOVRPose().position;
        else if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
    ...
    switch (controllerType)
    {
      ...
      case Controller.LHand:
        if (
            OVRNodeStateProperties.GetNodeStatePropertyVector3(
                Node.LeftHand,
                NodeStatePropertyType.Velocity,
                OVRPlugin.Node.HandLeft,
                stepType,
                out velocity
            )
    ...
    switch (controllerType)
    {
      ...
      case Controller.LHand:
        if (
            OVRNodeStateProperties.GetNodeStatePropertyVector3(
                Node.LeftHand,
                NodeStatePropertyType.Acceleration,
                OVRPlugin.Node.HandLeft,
                stepType,
                out accel
            )
    ...
    switch (controllerType)
    {
      ...
      case Controller.LHand:
        if (
            OVRNodeStateProperties.GetNodeStatePropertyVector3(
                Node.LeftHand,
                NodeStatePropertyType.AngularAcceleration,
                OVRPlugin.Node.HandLeft,
                stepType,
                out accel
            )
  ...
  )
  {
    ...
    for (int i = 0; i < controllers.Count; i++)
    {
      ...
      if (ShouldResolveController(controller.controllerType, controllerMask))
      {
        ...
        if (
            (((RawButton)controller.currentState.Buttons & resolvedMask) != 0)
  ...
  )
  {
    ...
    for (int i = 0; i < controllers.Count; i++)
    {
      ...
      if (ShouldResolveController(controller.controllerType, controllerMask))
      {
        ...
        if (
            (((RawTouch)controller.currentState.Touches & resolvedMask) != 0)
  ...
  )
  {
    ...
    for (int i = 0; i < controllers.Count; i++)
    {
      ...
      if (ShouldResolveController(controller.controllerType, controllerMask))
      {
        ...
        if (
            (((RawNearTouch)controller.currentState.NearTouches & resolvedMask) != 0)
  ...
  public static OpenVRControllerDetails[] openVRControllerDetails = new OpenVRControllerDetails[
      2
  ];
  ...
  /// </summary>
  public static void SetOpenVRLocalPose(
      Vector3 leftPos,
      Vector3 rightPos,
      Quaternion leftRot,
      Quaternion rightRot
  ...
  /// </summary>
  public static string GetOpenVRStringProperty(
      OVR.OpenVR.ETrackedDeviceProperty prop,
      uint deviceId = OVR.OpenVR.OpenVR.k_unTrackedDeviceIndex_Hmd
  )
  {
    OVR.OpenVR.ETrackedPropertyError error =
        OVR.OpenVR.ETrackedPropertyError.TrackedProp_Success;
    ...
    if (system != null)
    {
      ...
      return (error != OVR.OpenVR.ETrackedPropertyError.TrackedProp_Success)
          ? error.ToString()
          : "<unknown>";
    }
    return "";
    ...
    if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
    {
      ...
      if (system != null)
      {
        for (uint id = 0; id < OVR.OpenVR.OpenVR.k_unMaxTrackedDeviceCount; id++)
        {
          ...
          if (
              deviceClass == OVR.OpenVR.ETrackedDeviceClass.Controller
              && system.IsTrackedDeviceConnected(id)
          )
          {
            OpenVRController controllerType;
            String controllerName = GetOpenVRStringProperty(
                OVR.OpenVR.ETrackedDeviceProperty.Prop_ControllerType_String,
                id
            );
            ...
            OVR.OpenVR.ETrackedControllerRole role =
                system.GetControllerRoleForTrackedDeviceIndex(id);
            if (role == OVR.OpenVR.ETrackedControllerRole.LeftHand)
            {
              system.GetControllerState(
                  id,
                  ref openVRControllerDetails[0].state,
                  (uint)Marshal.SizeOf(typeof(OVR.OpenVR.VRControllerState_t))
              );
            ...
            else if (role == OVR.OpenVR.ETrackedControllerRole.RightHand)
            {
              system.GetControllerState(
                  id,
                  ref openVRControllerDetails[1].state,
                  (uint)Marshal.SizeOf(typeof(OVR.OpenVR.VRControllerState_t))
              );
  ...
  private static void PlayHapticImpulse(float amplitude, Node deviceNode)
  {
    ...
    {
      uint controllerId =
          (deviceNode == Node.LeftHand)
              ? openVRControllerDetails[0].deviceID
              : openVRControllerDetails[1].deviceID;

      if (IsValidOpenVRDevice(controllerId))
        system.TriggerHapticPulse(
            controllerId,
            0,
            (char)(OPENVR_MAX_HAPTIC_AMPLITUDE * amplitude)
        );
    }
  }
  ...
  )
  {
    ...
    else if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
    {
      ...
      {
        Node controllerNode =
            (controllerMask == Controller.LTouch) ? Node.LeftHand : Node.RightHand;
  ...
  /// </summary>
  public static byte GetControllerBatteryPercentRemaining(
      Controller controllerMask = Controller.Active
  )
  {
    if ((controllerMask & Controller.Active) != 0)
  ...
  )
  {
    ...
    if (
        ((controllerMask & Controller.Touch) == Controller.Touch)
    ...
    if (
        ((controllerMask & Controller.Hands) == Controller.Hands)
  ...
  private abstract class OVRControllerBase
  {
    ...
    private OVRPlugin.ControllerState4 GetOpenVRControllerState(Controller controllerType)
    {
      ...
      if (
          (controllerType & Controller.LTouch) == Controller.LTouch
          && IsValidOpenVRDevice(openVRControllerDetails[0].deviceID)
      )
      {
        OVR.OpenVR.VRControllerState_t leftControllerState =
            openVRControllerDetails[0].state;
        if (
            (leftControllerState.ulButtonPressed & ((ulong)OpenVRButton.Two))
            == (ulong)OpenVRButton.Two
        )
          state.Buttons |= (uint)RawButton.Y;
        if (
            (leftControllerState.ulButtonPressed & ((ulong)OpenVRButton.Thumbstick))
            == (ulong)OpenVRButton.Thumbstick
        )
        ...
        if (
            openVRControllerDetails[0].controllerType == OpenVRController.OculusTouch
            || openVRControllerDetails[0].controllerType == OpenVRController.ViveController
        )
        {
          ...
          state.LThumbstick.y = leftControllerState.rAxis0.y;
            ...
            openVRControllerDetails[0].controllerType
            == OpenVRController.WindowsMRController
        )
        {
        ...
        if (openVRControllerDetails[0].controllerType == OpenVRController.OculusTouch)
          ...
          state.LHandTrigger =
              (
                  (leftControllerState.ulButtonPressed & ((ulong)OpenVRButton.Grip))
                  == ((ulong)OpenVRButton.Grip)
              )
                  ? 1
                  : 0;
      }
      ...
      if (
          ...
          && IsValidOpenVRDevice(openVRControllerDetails[1].deviceID)
      )
      {
        OVR.OpenVR.VRControllerState_t rightControllerState =
            openVRControllerDetails[1].state;
        if (
            (rightControllerState.ulButtonPressed & ((ulong)OpenVRButton.Two))
            == (ulong)OpenVRButton.Two
        )
          state.Buttons |= (uint)RawButton.B;
        if (
            (rightControllerState.ulButtonPressed & ((ulong)OpenVRButton.Thumbstick))
            == (ulong)OpenVRButton.Thumbstick
        )
        ...
        if (
            openVRControllerDetails[1].controllerType == OpenVRController.OculusTouch
            || openVRControllerDetails[1].controllerType == OpenVRController.ViveController
        )
        {
          ...
          state.RThumbstick.y = rightControllerState.rAxis0.y;
            ...
            openVRControllerDetails[1].controllerType
            == OpenVRController.WindowsMRController
        )
        {
        ...
        if (openVRControllerDetails[1].controllerType == OpenVRController.OculusTouch)
          ...
          state.RHandTrigger =
              (
                  (rightControllerState.ulButtonPressed & ((ulong)OpenVRButton.Grip))
                  == ((ulong)OpenVRButton.Grip)
              )
                  ? 1
                  : 0;
