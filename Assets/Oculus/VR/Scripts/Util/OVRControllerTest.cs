...
public class OVRControllerTest : MonoBehaviour
{
    ...
    monitors = new List<BoolMonitor>()
        {
            ...
            new BoolMonitor(
                "PrimaryIndexTrigger (Touch)",
                () => OVRInput.Get(OVRInput.Touch.PrimaryIndexTrigger)
            ),
            new BoolMonitor(
                "PrimaryIndexTriggerDown (Touch)",
                () => OVRInput.GetDown(OVRInput.Touch.PrimaryIndexTrigger)
            ),
            new BoolMonitor(
                "PrimaryIndexTriggerUp (Touch)",
                () => OVRInput.GetUp(OVRInput.Touch.PrimaryIndexTrigger)
            ),
            new BoolMonitor(
                "PrimaryHandTrigger",
  ...
  static BoolMonitor controllers = new BoolMonitor(
      "Controllers Changed",
      () =>
      {
  ...
  void Update()
  {
    ...
    data.AppendFormat(
        "Orientation: ({0:F2}, {1:F2}, {2:F2}, {3:F2})\n",
        rot.x,
        rot.y,
        rot.z,
        rot.w
