...
public class OVRVignette : MonoBehaviour
{
  ...
  [SerializeField]
  [Tooltip(
      "Controls the number of triangles used for the vignette mesh."
  ...
  [Tooltip(
      "The Aspect ratio of the vignette controls the "
  ...
  private int GetTriangleCount()
  {
    ...
    {
      case MeshComplexityLevel.VerySimple:
        return 32;
      case MeshComplexityLevel.Simple:
        return 64;
      case MeshComplexityLevel.Normal:
        return 128;
      case MeshComplexityLevel.Detailed:
        return 256;
      case MeshComplexityLevel.VeryDetailed:
        return 512;
      default:
        return 128;
    }
  ...
  private void Awake()
  {
    ...
    GameObject opaqueObject = new GameObject("Opaque Vignette")
    {
      hideFlags = HideFlags.HideAndDontSave
    };
    ...
    GameObject transparentObject = new GameObject("Transparent Vignette")
    {
      hideFlags = HideFlags.HideAndDontSave
    };
  ...
  private void GetTanFovAndOffsetForStereoEye(
  ...
  private void GetTanFovAndOffsetForMonoEye(
      out float tanFovX,
      out float tanFovY,
      out float offsetX,
      out float offsetY
  ...
  private bool VisibilityTest(float scaleX, float scaleY, float offsetX, float offsetY)
  {
    ...
    return new Vector2(
            (1 + Mathf.Abs(offsetX)) / scaleX,
            (1 + Mathf.Abs(offsetY)) / scaleY
        ).sqrMagnitude > 1.0f;
  }
  ...
  private void Update()
  {
    ...
    float tanMiddleFovX = Mathf.Tan(
        (VignetteFieldOfView + VignetteFalloffDegrees) * Mathf.Deg2Rad * 0.5f
    );
    ...
    for (int i = 0; i < 2; i++)
    {
      ...
      float borderScale =
          new Vector2(
              (1 + Mathf.Abs(offsetX)) / VignetteAspectRatio,
              1 + Mathf.Abs(offsetY)
          ).magnitude * 1.01f;
      ...
      // test for visibility.
      _TransparentVignetteVisible |= VisibilityTest(
          innerScaleX,
          innerScaleY,
          offsetX,
          offsetY
      ...
      _TransparentScaleAndOffset0[i] = new Vector4(
          middleScaleX,
          middleScaleY,
          offsetX,
          offsetY
      );
      _TransparentScaleAndOffset1[i] = new Vector4(
          innerScaleX,
          innerScaleY,
          offsetX,
          offsetY
    ...
    _TransparentMaterial.SetVectorArray(
        _ShaderScaleAndOffset0Property,
        _TransparentScaleAndOffset0
    );
    _TransparentMaterial.SetVectorArray(
        _ShaderScaleAndOffset1Property,
