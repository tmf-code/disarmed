    ...
    get
    {
      if (_instance == null)
      {
        ...
        _instance = (OVRGazePointer)GameObject.Instantiate(
            (OVRGazePointer)Resources.Load(
                "Prefabs/GazePointerRing",
                typeof(OVRGazePointer)
            )
        );
      }
      return _instance;
  ...
  public float visibilityStrength
  {
    get
    {
      ...
      if (hideByDefault)
      {
        ...
        strengthFromShowRequest = Mathf.Clamp01(
            1 - (Time.time - lastShowRequestTime) / showTimeoutPeriod
        );
      }
      else
      {
      ...
      strengthFromHideRequest =
          (lastHideRequestTime + hideTimeoutPeriod > Time.time)
