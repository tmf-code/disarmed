...
namespace UnityEngine.EventSystems
{
  ...
  public class OVRInputModule : PointerInputModule
  {
    ...
    private void ProcessMouseEvent(MouseState mouseData)
    {
      ...
      var leftButtonData =
          mouseData.GetButtonState(PointerEventData.InputButton.Left).eventData;
    ...
    protected Dictionary<int, OVRPointerEventData> m_VRRayPointerData = new Dictionary<
        int,
        OVRPointerEventData
    >();
    ...
    protected bool GetPointerData(int id, out OVRPointerEventData data, bool create)
    {
      ...
      {
        data = new OVRPointerEventData(eventSystem) { pointerId = id, };
    ...
    private bool ShouldStartDrag(PointerEventData pointerEvent)
    {
      ...
      else
      {
                ...
                if (
                    useSwipeScroll
                    && ((Vector3)pointerEvent.GetSwipeStart() - Input.mousePosition).magnitude
                        > swipeDragThreshold
                ) {
                    return true;
                }
        ...
        Vector3 pressDir =
            (pointerEvent.pointerPressRaycast.worldPosition - cameraPos).normalized;
        Vector3 currentDir =
            (pointerEvent.pointerCurrentRaycast.worldPosition - cameraPos).normalized;
        return Vector3.Dot(pressDir, currentDir)
            < Mathf.Cos(Mathf.Deg2Rad * (angleDragThreshold));
      }
    }
    ...
    protected Vector2 GetExtraScrollDelta()
    {
      ...
      if (useRightStickScroll)
      {
        ...
        if (Mathf.Abs(s.x) < rightStickDeadZone)
          s.x = 0;
        if (Mathf.Abs(s.y) < rightStickDeadZone)
          s.y = 0;
        scrollDelta = s;
      }
      return scrollDelta;
