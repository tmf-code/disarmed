...
public class OVRNetwork
{
  ...
  public class OVRNetworkTcpServer
  {
    ...
    public void StartListening(int listeningPort)
    {
      ...
      try
      {
        tcpListener.Start();
        // Debug.LogFormat(
            // "TcpListener started. Local endpoint: {0}",
      ...
      catch (SocketException e)
      {
        Debug.LogWarningFormat(
            "[OVRNetworkTcpServer] Unsable to start TcpListener. Socket exception: {0}",
            e.Message
        );
        Debug.LogWarning(
      ...
      if (tcpListener != null)
      {
        ...
        {
          Debug.LogWarningFormat(
              "[OVRNetworkTcpServer] can't accept new client: {0}",
              e.Message
          );
        }
      }
    ...
    private void DoAcceptTcpClientCallback(IAsyncResult ar)
    {
      ...
      {
        Debug.LogWarningFormat(
            "[OVRNetworkTcpServer] EndAcceptTcpClient failed: {0}",
            e.Message
        );
      }
    }
    ...
    public void Broadcast(int payloadType, byte[] payload)
    {
      ...
      {
        Debug.LogWarningFormat(
            "[OVRNetworkTcpServer] drop payload because it's too long: {0} bytes",
            payload.Length
        );
      }
      ...
      lock (clientsLock)
      {
        foreach (TcpClient client in clients)
        {
          if (client.Connected)
          {
            try
            {
              client.GetStream()
                  .BeginWrite(
                      ...
                      0,
                      dataBuffer.Length,
                      new AsyncCallback(DoWriteDataCallback),
                      client.GetStream()
                  );
            }
            catch (SocketException e)
            {
              Debug.LogWarningFormat(
                  "[OVRNetworkTcpServer] close client because of socket error: {0}",
                  e.Message
              );
  ...
  public class OVRNetworkTcpClient
  {
    ...
    {
            new byte[OVRNetwork.MaxBufferLength],
            new byte[OVRNetwork.MaxBufferLength]
        };
    ...
    public void Tick()
    {
      ...
      if (readyReceiveDataEvent.WaitOne(TimeSpan.Zero))
      {
        if (tcpClient.GetStream().DataAvailable)
        {
          ...
          int maximumDataSize =
              OVRSystemPerfMetrics.MaxBufferLength - receivedBufferDataSize;

          tcpClient.GetStream()
              .BeginRead(
                  receivedBuffers[receivedBufferIndex],
                  receivedBufferDataSize,
                  maximumDataSize,
                  new AsyncCallback(OnReadDataCallback),
                  tcpClient.GetStream()
              );
        }
      }
    ...
    void OnReadDataCallback(IAsyncResult ar)
    {
      ...
      try
      {
        ...
        {
          FrameHeader header = FrameHeader.FromBytes(
              receivedBuffers[receivedBufferIndex]
          );
          ...
          if (
              ...
              || header.payloadLength > OVRNetwork.MaxPayloadLength
          )
          {
            Debug.LogWarningFormat(
                "[OVRNetworkTcpClient] Sanity check failed. PayloadLength %d",
          ...
          if (receivedBufferDataSize >= FrameHeader.StructSize + header.payloadLength)
          {
            ...
            if (newBufferDataSize > 0)
            {
              Array.Copy(
                  receivedBuffers[receivedBufferIndex],
                  (FrameHeader.StructSize + header.payloadLength),
                  receivedBuffers[newBufferIndex],
                  0,
                  newBufferDataSize
              );
            }
      ...
      catch (SocketException e)
      {
        Debug.LogErrorFormat(
            "[OVRNetworkTcpClient] OnReadDataCallback: socket error: {0}",
            e.Message
        );
        Disconnect();
      }
