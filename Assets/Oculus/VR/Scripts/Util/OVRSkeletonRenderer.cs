...
public class OVRSkeletonRenderer : MonoBehaviour
{
  ...
  private class CapsuleVisualization
  {
    ...
    )
    {
      ...
      CapsuleGO.transform.rotation =
          BoneCapsule.CapsuleCollider.transform.rotation * _capsuleRotationOffset;
      CapsuleGO.transform.position = BoneCapsule.CapsuleCollider.transform.TransformPoint(
          BoneCapsule.CapsuleCollider.center
      );
      CapsuleGO.transform.localScale = capsuleScale * scale;
      ...
      if (systemGestureBehavior == SystemGestureBehavior.SwapMaterial)
      {
        ...
        )
        {
          Renderer.sharedMaterial = SystemGestureMaterial;
            ...
            !shouldUseSystemGestureMaterial && Renderer.sharedMaterial != RenderMaterial
        )
        {
          Renderer.sharedMaterial = RenderMaterial;
        }
  ...
  public void Update()
  {
    ...
    if (IsInitialized)
    {
      ...
      {
        _capsuleVisualizations[i].Update(
            _scale,
            shouldRender,
            ShouldUseSystemGestureMaterial,
            _confidenceBehavior,
