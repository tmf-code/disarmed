...
public class OVRSpectatorModeDomeTest : MonoBehaviour
{
  ...
  public Transform Head;
    ...
    private OVRPlugin.Media.PlatformCameraMode camMode =
        OVRPlugin.Media.PlatformCameraMode.Disabled;
  ...
  void Initialize()
  {
    ...
    Debug.LogFormat(
        "GetExternalCameraCount before adding manual external camera {0}",
        OVRPlugin.GetExternalCameraCount()
    );
    UpdateDefaultExternalCamera();
    Debug.LogFormat(
        "GetExternalCameraCount after adding manual external camera {0}",
        OVRPlugin.GetExternalCameraCount()
    );
  ...
  void UpdateDefaultExternalCamera()
  {
    ...
    if (
        !OVRPlugin.SetDefaultExternalCamera(
            cameraName,
  ...
  private void UpdateSpectatorCameraStatus()
  {
        ...
        if (
            ...
            if (camMode == OVRPlugin.Media.PlatformCameraMode.Disabled && readyToSwitch)
            {
                ...
                OVRPlugin.Media.SetPlatformCameraMode(
        ...
        SpectatorCamera.position = SpectatorCameraDomePosition(
            SpectatorAnchor.position,
            distance,
            elevation,
            polar
        );
        SpectatorCamera.rotation = Quaternion.LookRotation(
            SpectatorCamera.position - SpectatorAnchor.position
        );
  ...
  Vector3 SpectatorCameraDomePosition(Vector3 spectatorAnchorPosition, float d, float e, float p)
  {
    ...
    return new Vector3(
        x + spectatorAnchorPosition.x,
        y + spectatorAnchorPosition.y,
        z + spectatorAnchorPosition.z
    );
  }
