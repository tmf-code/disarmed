...
public class OVRPlayerController : MonoBehaviour
{
  ...
  /// </summary>
  [Tooltip(
      "[Deprecated] When enabled, snap rotation will happen about the center of the "
  ...
  protected virtual void UpdateController()
  {
    if (useProfileData)
    {
      ...
      {
        p.y =
            OVRManager.profile.eyeHeight - (0.5f * Controller.height) + Controller.center.y;
      }
    ...
    if (Controller.isGrounded && FallSpeed <= 0)
      ...
      FallSpeed += (
          (Physics.gravity.y * (GravityModifier * 0.002f)) * SimulationRate * Time.deltaTime
      );
    ...
    Vector3 predictedXZ = Vector3.Scale(
  ...
  public virtual void UpdateMovement()
  {
    ...
    if (EnableLinearMovement)
    {
      ...
      if (moveBack)
        MoveThrottle +=
            ort
            * (transform.lossyScale.z * moveInfluence * BackAndSideDampen * Vector3.back);
      if (moveLeft)
        MoveThrottle +=
            ort
            * (transform.lossyScale.x * moveInfluence * BackAndSideDampen * Vector3.left);
      if (moveRight)
        MoveThrottle +=
            ort
            * (transform.lossyScale.x * moveInfluence * BackAndSideDampen * Vector3.right);
      ...
      if (primaryAxis.y > 0.0f)
        MoveThrottle +=
            ort
            * (primaryAxis.y * transform.lossyScale.z * moveInfluence * Vector3.forward);

      if (primaryAxis.y < 0.0f)
        MoveThrottle +=
            ort
            * (
                Mathf.Abs(primaryAxis.y)
    ...
    if (EnableRotation)
    {
      Vector3 euler = RotateAroundGuardianCenter
          ? transform.rotation.eulerAngles
          : Vector3.zero;
      float rotateInfluence =
          SimulationRate * Time.deltaTime * RotationAmount * RotationScaleMultiplier;
