...
public class OVRSceneLoader : MonoBehaviour
{
  ...
  private void LoadScene(SceneInfo sceneInfo)
  {
    ...
    Debug.Log(
        "[OVRSceneLoader] Loading main scene: "
            + sceneInfo.scenes[0]
            + " with version "
    ...
    try
    {
      foreach (string b in bundles)
      {
        ...
        {
          Debug.Log(
              "[OVRSceneLoader] Loading file bundle: " + assetBundle.name == null
                  ? "null"
                  : assetBundle.name
          );
  ...
  public void Update()
  {
    ...
    if (loadSceneOperation != null)
    {
      if (!loadSceneOperation.isDone)
      {
        logTextBox.text = string.Format(
            formattedLogText,
            loadSceneOperation.progress + 0.1f
        );
        if (loadSceneOperation.progress >= 0.9f)
        {
          logTextBox.text = formattedLogText.Replace(
              "{0:P0}",
    ...
    if (closeLogDialogue)
    {
      if (closeLogTimer < logCloseTime)
      {
        ...
        logTextBox.text = string.Format(
            formattedLogText,
            (int)(logCloseTime - closeLogTimer)
        );
      }
      else
      {
