...
public class OculusSpatializerUnity : MonoBehaviour
{
  ...
  static Vector3 swapHandedness(Vector3 vec)
  {
    return new Vector3(vec.x, vec.y, -vec.z);
  }
  [MonoPInvokeCallback(typeof(AudioRaycastCallback))]
  static void AudioRaycast(
      Vector3 origin,
      Vector3 direction,
      out Vector3 point,
      out Vector3 normal,
  ...
  )
  {
    ...
    if (
        Physics.Raycast(
            swapHandedness(origin),
            swapHandedness(direction),
            out hitInfo,
            1000.0f,
            gLayerMask.value
        )
      ...
      Texture2D decalTex;
      {
        ...
        for (int i = 0; i < SIZE / 2; ++i)
        {
          for (int j = 0; j < SIZE / 2; ++j)
          {
            ...
            float alpha =
                (dist < HALF_SIZE)
                    ? Mathf.Clamp01(Mathf.Sin(Mathf.PI * 2.0f * t))
                    : 0.0f;
  ...
  // * * * * * * * * * * * * *
  // Import functions
  public delegate void AudioRaycastCallback(
      Vector3 origin,
      Vector3 direction,
      out Vector3 point,
      out Vector3 normal,
  ...
  [DllImport(strOSP)]
  private static extern int OSP_Unity_AssignRaycastCallback(
      AudioRaycastCallback callback,
      System.IntPtr data
  );
  [DllImport(strOSP)]
  private static extern int OSP_Unity_AssignRaycastCallback(
  ...
  [DllImport(strOSP)]
  private static extern int OSP_Unity_GetRoomDimensions(
      float[] roomDimensions,
      float[] reflectionsCoefs,
      out Vector3 position
  );
  [DllImport(strOSP)]
  private static extern int OSP_Unity_GetRaycastHits(
      Vector3[] points,
      Vector3[] normals,
      int length
  );
