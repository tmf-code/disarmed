  ...
  void OnEnable()
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          Debug.Log(
              "Ambisonic ERROR: AudioSource clip does not have correct number of channels."
          );
        }
      }
    }
    ...
    if (status != currentStatus)
    {
      switch (status)
      {
        case (ovrAmbisonicsNativeStatus.NotEnabled):
          Debug.Log(
              "Ambisonic Native: Ambisonic not enabled on clip. Check clip field and turn it on"
          );
          break;
        ...
        case (ovrAmbisonicsNativeStatus.Success):
          Debug.Log(
              "Ambisonic Native: Stream successfully initialized and playing/playable"
          );
          break;
        ...
        case (ovrAmbisonicsNativeStatus.ProcessError):
          Debug.Log(
              "Ambisonic Native WARNING: Stream process error (check default speaker setup)"
          );
          break;
