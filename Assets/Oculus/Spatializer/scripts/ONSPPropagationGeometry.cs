...
public class ONSPPropagationGeometry : MonoBehaviour
{
  ...
  {
    get { return Application.streamingAssetsPath + "/" + GeometryAssetDirectory; }
  }
  //-------
  // PUBLIC
  ...
  {
    get { return GeometryAssetPath + "/" + filePathRelative; }
  }
  ...
  private static void traverseMeshHierarchy(
      GameObject obj,
      ONSPPropagationMaterial[] currentMaterials,
      bool includeChildren,
      List<MeshMaterial> meshMaterials,
      List<TerrainMaterial> terrainMaterials,
      bool ignoreStatic,
  ...
  )
  {
    ...
    foreach (MeshFilter meshFilter in meshes)
    {
      ...
      {
        Debug.LogWarning(
            "Mesh: " + meshFilter.gameObject.name + " not readable, cannot be static.",
  ...
  )
  {
      ...
      if (treePrototypes.Length != 0)
      {
        ...
        foreach (TreeInstance tree in trees)
        {
          updateCountsForMesh(
              ...
              ref totalFaceCount,
              ref totalMaterialCount,
              t.treePrototypeMeshes[tree.prototypeIndex]
          );
        }
    ...
    foreach (TerrainMaterial t in terrains)
    {
      ...
      meshScale = new Vector3(
          meshScale.x / (w - 1) * terrainDecimation,
          meshScale.y,
      ...
      for (int y = 0; y < hRes; y++)
      {
        for (int x = 0; x < wRes; x++)
        {
          ...
          Vector3 v = matrix.MultiplyPoint3x4(
              Vector3.Scale(
                  meshScale,
      ...
      foreach (TreeInstance tree in trees)
      {
        ...
        Matrix4x4 treeLocalToWorldMatrix =
            t.terrain.gameObject.transform.localToWorldMatrix;
        treeLocalToWorldMatrix.SetColumn(
            3,
    ...
    // Upload mesh data
    return ONSPPropagation.Interface.AudioGeometryUploadMeshArrays(
        geometryHandle,
        vertices,
        totalVertexCount,
        indices,
  ...
  private static void uploadMeshFilter(
      ...
      ref int groupOffset,
      Mesh mesh,
      ONSPPropagationMaterial[] materials,
      Matrix4x4 matrix
  )
  {
  ...
  public void UploadGeometry()
  {
    ...
    {
      Debug.LogError(
          "Failed to upload meshes, "
              + ignoredMeshCount
              + " static meshes ignored. Turn on \"File Enabled\" to process static meshes offline",
          gameObject
      );
    }
  }
  ...
  public bool WriteFile()
  {
    ...
    if (
        uploadMesh(tempGeometryHandle, gameObject, gameObject.transform.worldToLocalMatrix)
        != OSPSuccess
    )
    {
    ...
    if (
        ONSPPropagation.Interface.AudioGeometryWriteMeshFile(tempGeometryHandle, filePath)
        != OSPSuccess
    )
    {
  ...
  public bool ReadFile()
  {
    ...
    if (
        ONSPPropagation.Interface.AudioGeometryReadMeshFile(geometryHandle, filePath)
        != OSPSuccess
    )
    {
  ...
  public bool WriteToObj()
  {
    ...
    if (
        ONSPPropagation.Interface.AudioGeometryWriteMeshFileObj(
            tempGeometryHandle,
            filePath + ".obj"
        ) != OSPSuccess
    )
