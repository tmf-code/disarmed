...
namespace OculusSampleFramework
{
  ...
  public class DistanceGrabber : OVRGrabber
  {
    ...
    protected override void Start()
    {
      ...
      OVRPlugin.SendEvent(
          "distance_grabber",
          (SceneManager.GetActiveScene().name == "DistanceGrab").ToString(),
          "sample_framework"
      );
#endif
    }
    ...
    protected override void GrabBegin()
    {
      ...
      if (closestGrabbable != null)
      {
        ...
        if (
            ...
            && (closestPointOnBounds - m_gripTransform.position).magnitude
                < m_noSnapThreshhold
        )
        {
    ...
    static private DistanceGrabbable HitInfoToGrabbable(RaycastHit hitInfo)
    {
      if (hitInfo.collider != null)
      {
        ...
        return go.GetComponent<DistanceGrabbable>()
            ?? go.GetComponentInParent<DistanceGrabbable>();
      }
      return null;
    }
      ...
      foreach (OVRGrabbable cg in m_grabCandidates.Keys)
      {
        ...
        for (int j = 0; j < grabbable.grabPoints.Length; ++j)
        {
          ...
          float grabbableMagSq =
              (m_gripTransform.position - closestPointOnBounds).sqrMagnitude;
          ...
          if (grabbableMagSq < closestMagSq)
          {
            ...
            if (m_preventGrabThroughWalls)
            {
              ...
              )
              {
                float distToObject =
                    (
                        grabbableCollider.ClosestPointOnBounds(
                            m_gripTransform.position
                        ) - m_gripTransform.position
                    ).magnitude;
    ...
    protected bool FindTargetWithSpherecast(out DistanceGrabbable dgOut, out Collider collOut)
    {
      ...
      if (Physics.SphereCast(ray, m_spherecastRadius, out hitInfo, m_maxGrabDistance, layer))
      {
        ...
        {
          grabbable =
              hitInfo.collider.gameObject.GetComponentInParent<DistanceGrabbable>();
        ...
        if (grabbable != null && m_preventGrabThroughWalls)
        {
          ...
          )
          {
            ...
            if (
                obstruction != grabbable
                && obstructionHitInfo.distance < hitInfo.distance
            )
            {
              dgOut = null;
              collOut = null;
