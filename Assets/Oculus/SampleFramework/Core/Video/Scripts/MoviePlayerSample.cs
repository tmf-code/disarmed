...
public class MoviePlayerSample : MonoBehaviour
{
  ...
  private void UpdateShapeAndStereo()
  {
    ...
    if (Shape != _LastShape || Stereo != _LastStereo || DisplayMono != _LastDisplayMono)
    {
      ...
      overlay.SetSrcDestRects(
          sourceLeft,
          DisplayMono ? sourceLeft : sourceRight,
          destRect,
          destRect
      );
    ...
    if (!string.IsNullOrEmpty(MovieName))
    {
      if (IsLocalVideo(MovieName))
      {
        ...
        var guids = UnityEditor.AssetDatabase.FindAssets(
            Path.GetFileNameWithoutExtension(MovieName)
        );
  ...
  void Update()
  {
    ...
    {
      var displayTexture =
          videoPlayer.texture != null ? videoPlayer.texture : Texture2D.blackTexture;
      if (overlay.enabled)
      {
...
#else
            Duration =
                videoPlayer.frameRate > 0
                    ? (long)(videoPlayer.frameCount / videoPlayer.frameRate * 1000L)
                    : 0L;
#endif
    }
    else
