...
namespace OculusSampleFramework
{
  ...
  {
    public InteractableCollisionInfo(
  ...
  public abstract class InteractableTool : MonoBehaviour
  {
    ...
    public abstract void FocusOnInteractable(
    ...
    private Dictionary<
        Interactable,
        InteractableCollisionInfo
    > _currInteractableToCollisionInfos = new Dictionary<
        Interactable,
        InteractableCollisionInfo
    >();
    private Dictionary<
        Interactable,
        InteractableCollisionInfo
    > _prevInteractableToCollisionInfos = new Dictionary<
        Interactable,
        InteractableCollisionInfo
    >();
    ...
    public virtual void UpdateCurrentCollisionsBasedOnDepth()
    {
      _currInteractableToCollisionInfos.Clear();
      foreach (
          InteractableCollisionInfo interactableCollisionInfo in _currentIntersectingObjects
      )
      {
        var interactable =
            interactableCollisionInfo.InteractableCollider.ParentInteractable;
        var depth = interactableCollisionInfo.CollisionDepth;
        InteractableCollisionInfo collisionInfoFromMap = null;

        if (
            !_currInteractableToCollisionInfos.TryGetValue(
                interactable,
                out collisionInfoFromMap
            )
        )
        {
        ...
        else if (collisionInfoFromMap.CollisionDepth < depth)
        {
          collisionInfoFromMap.InteractableCollider =
              interactableCollisionInfo.InteractableCollider;
    ...
    public virtual void UpdateLatestCollisionData()
    {
      ...
      foreach (Interactable addedInteractableKey in _addedInteractables)
      {
        ...
        addedInteractableKey.UpdateCollisionDepth(
            this,
            InteractableCollisionDepth.None,
            collisionDepth
        );
      ...
      {
        var newDepth =
            _currInteractableToCollisionInfos[remainingInteractableKey].CollisionDepth;
        var oldDepth =
            _prevInteractableToCollisionInfos[remainingInteractableKey].CollisionDepth;
