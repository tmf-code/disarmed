...
namespace OculusSampleFramework
{
  ...
  public class OVROverlaySample : MonoBehaviour
  {
    ...
    void Start()
    {
      ...
      DebugUIBuilder.instance.AddRadio(
              "OVROverlay",
              "group",
              delegate (Toggle t)
              {
      ...
      applicationRadioButton = DebugUIBuilder.instance.AddRadio(
              "Application",
              "group",
              delegate (Toggle t)
              {
          ...
          .GetComponentInChildren<Toggle>();
      noneRadioButton = DebugUIBuilder.instance.AddRadio(
              "None",
              "group",
              delegate (Toggle t)
              {
    ...
    IEnumerator WaitforOVROverlay()
    {
      ...
      Vector3 newPos =
          camTransform.position + camTransform.forward * distanceFromCamToLoadText;
    ...
    void CameraAndRenderTargetSetup()
    {
      ...
      float screenSizeXInWorld =
          screenSizeYInWorld * mainCamera.GetComponent<Camera>().aspect;
      ...
      Debug.Log(
          "Created RT of resolution w: " + renderTargetWidth + " and h: " + renderTargetHeight
      );
    ...
    void SimulateLevelLoad()
    {
      ...
      for (int j = 0; j < numLevels; j++)
      {
        for (var i = 0; i < numObjectsPerLevel; i++)
        {
          ...
          var newInst = Instantiate(
              prefabForLevelLoadSim,
              pos + playerPos,
              Quaternion.identity
          );
