...
public class OVROverlayCanvasEditor : Editor
{
  public override void OnInspectorGUI()
  {
    ...
    canvas.MaxTextureSize = EditorGUILayout.IntField(
        new GUIContent(
            "Max Texture Size",
            "Limits the maximum size of the texture used for this canvas"
        ),
        canvas.MaxTextureSize
    );
    canvas.MinTextureSize = EditorGUILayout.IntField(
        new GUIContent(
            "Min Texture Size",
            "Limits the minimum size this texture will be displayed at"
        ),
    ...
    canvas.PixelsPerUnit = EditorGUILayout.FloatField(
        new GUIContent("Pixels Per Unit", "Controls the density of the texture"),
    ...
    canvas.DrawRate = EditorGUILayout.IntField(
        new GUIContent(
            "Draw Rate",
            "Controls how frequently this canvas updates. A value of 1 means every frame, 2 means every other, etc."
        ),
        canvas.DrawRate
    );
    if (canvas.DrawRate > 1)
    {
      canvas.DrawFrameOffset = EditorGUILayout.IntField(
          new GUIContent(
              "Draw Frame Offset",
    ...
    canvas.Expensive = EditorGUILayout.Toggle(
        new GUIContent(
            "Expensive",
            "Improve the visual appearance at the cost of additional GPU time"
        ),
        canvas.Expensive
    );
    canvas.Opacity = (OVROverlayCanvas.DrawMode)EditorGUILayout.EnumPopup(
        new GUIContent(
            "Opacity",
            "Treat this canvas as opaque, which is a big performance improvement"
        ),
        canvas.Opacity
    );
        ...
        if (GUILayout.Button("Replace Materials"))
        {
          ...
          if (matList.Length > 0)
          {
            var mat = AssetDatabase.LoadAssetAtPath<Material>(
                AssetDatabase.GUIDToAssetPath(matList[0])
            );
    ...
    if (
        canvas.Opacity == OVROverlayCanvas.DrawMode.TransparentCorrectAlpha
    ...
    )
    {
      if (PlayerSettings.colorSpace == ColorSpace.Gamma)
      {
        ...
        GUI.contentColor = Color.yellow;
        EditorGUILayout.LabelField(
            "Alpha blending may not be correct with Gamma Color Space"
        );
        GUI.contentColor = prevColor;
      }
    }

    canvas.Layer = EditorGUILayout.LayerField(
        new GUIContent("Layer", "The layer this overlay should be drawn on"),
        canvas.Layer
    );
    ...
    if (Camera.main != null)
    {
      if ((Camera.main.cullingMask & (1 << canvas.gameObject.layer)) != 0)
      {
        ...
        EditorGUILayout.LabelField(
            "Main Camera does not cull "
                + LayerMask.LayerToName(canvas.gameObject.layer)
                + ". Make sure the layer of this object is not drawn by the main camera"
        );
        GUI.contentColor = prevColor;
      }
      if ((Camera.main.cullingMask & (1 << canvas.Layer)) == 0)
      {
        ...
        GUI.contentColor = Color.red;
        EditorGUILayout.LabelField(
            "Layer should be assigned to a layer visible to your main camera."
        );
        GUI.contentColor = prevColor;
      }
    }
    else
    {
      ...
      GUI.contentColor = Color.yellow;
      EditorGUILayout.LabelField(
          "No Main Camera found. Make sure you camera does not draw layer "
              + LayerMask.LayerToName(canvas.gameObject.layer)
      );
      GUI.contentColor = prevColor;
    }
    ...
    if (canvas.Layer == canvas.gameObject.layer)
    {
      ...
      GUI.contentColor = Color.red;
      EditorGUILayout.LabelField(
          "Layer is set to the same layer as this object ("
              + LayerMask.LayerToName(canvas.gameObject.layer)
              + ")."
      );
      GUI.contentColor = prevColor;
    ...
    if (Application.isPlaying)
    {
      ...
      canvas.overlayEnabled = EditorGUILayout.Toggle(
          "Overlay Enabled",
