...
public class OVROverlayCanvas : MonoBehaviour
{
  ...
  void Start()
  {
    ...
    Vector2 opaqueTrim =
        Opacity == DrawMode.Opaque
            ? new Vector2(
                  0.005f / _rectTransform.lossyScale.x,
                  0.005f / _rectTransform.lossyScale.y
              )
            : Vector2.zero;
    ...
    _quad = new Mesh() { name = name + " Overlay Quad", hideFlags = HideFlags.HideAndDontSave };
    ...
    _defaultMat.mainTextureOffset = new Vector2(
        0.5f - 0.5f * insetRectWidth,
        0.5f - 0.5f * insetRectHeight
    );
    ...
    meshRenderer.transform.localScale = new Vector3(
        rectWidth - opaqueTrim.x,
        rectHeight - opaqueTrim.y,
        1
    );
  ...
  protected virtual bool ShouldRender()
  {
    ...
    if (Camera.main != null)
    {
      ...
      for (int i = 0; i < 2; i++)
      {
        ...
        var mat =
            Camera.main.GetStereoProjectionMatrix(eye)
            * Camera.main.GetStereoViewMatrix(eye);
      ...
      if (ScaleViewport)
      {
        if (Camera.main != null)
        {
          ...
          float size =
              PixelsPerUnit
              * Mathf.Max(
                  _rectTransform.rect.width * transform.lossyScale.x,
                  _rectTransform.rect.height * transform.lossyScale.y
              )
              / d;
          ...
          float pixelHeight =
              Mathf.Ceil(size / quantize * _renderTexture.height) * quantize;
          ...
          Rect src = new Rect(
              0.5f - (0.5f * innerSizeX),
              0.5f - (0.5f * innerSizeY),
              innerSizeX,
              innerSizeY
          );
