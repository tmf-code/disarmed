...
public class HandMeshMask : MonoBehaviour
{
  ...
  [Tooltip(
      "Along the fingers, each knuckle scales down by this amount.  Default is zero for uniform width along entire finger."
  )]
  public float fingerTaper = 0.13f;
  [Tooltip(
      "Shorten the last bone of each finger; need this to account for bone structure (end bone is at finger tip instead of center). Default is 1."
  )]
  public float fingerTipLength = 0.8f;
  [Tooltip(
      "Move the base of the 4 main fingers towards the tips, to avoid a visible mesh crack between finger webbing. Default is 0."
  )]
  ...
  void CreateHandMesh()
  {
    ...
    for (int i = 0; i < 5; i++)
    {
      ...
      AddKnuckleMesh(
          ...
          k0taper,
          k1taper,
          referenceHand.Bones[baseId].Transform.position,
  ...
  void AddPalmMesh(int knuckleVerts)
  {
    ...
    Vector3 customVert1 =
        (referenceHand.Bones[9].Transform.position + referenceHand.Bones[12].Transform.position)
        * 0.5f;
    // vertex at "saddle" between thumb and index
    Vector3 customVert2 =
        (referenceHand.Bones[4].Transform.position + referenceHand.Bones[6].Transform.position)
        * 0.5f;
    customVert2 =
        (customVert2 - referenceHand.Bones[15].Transform.position) * 0.9f
        + referenceHand.Bones[15].Transform.position;
    // vertex further up thumb, between bones 4 and 5
    Vector3 thumbPos =
        (referenceHand.Bones[5].Transform.position - referenceHand.Bones[4].Transform.position)
        * webOffset;
    ...
    Vector3 indexPos =
        (referenceHand.Bones[7].Transform.position - referenceHand.Bones[6].Transform.position)
        * webOffset;
    indexPos += referenceHand.Bones[6].Transform.position;
    Vector3 pinkyPos =
        (
            referenceHand.Bones[17].Transform.position
            - referenceHand.Bones[16].Transform.position
        ) * webOffset;
    pinkyPos += referenceHand.Bones[16].Transform.position;
    Vector3 middlePos =
        (referenceHand.Bones[10].Transform.position - referenceHand.Bones[9].Transform.position)
        + (
            referenceHand.Bones[13].Transform.position
            - referenceHand.Bones[12].Transform.position
        );
    ...
    AddKnuckleMesh(
        ...
        1.3f,
        1.2f,
        referenceHand.Bones[0].Transform.position,
        referenceHand.Bones[15].Transform.position
    );
    AddKnuckleMesh(
        knuckleVerts,
