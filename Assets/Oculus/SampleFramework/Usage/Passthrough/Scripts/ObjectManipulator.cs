...
public class ObjectManipulator : MonoBehaviour
{
  ...
  {
    localGrabOffset =
        Quaternion.Inverse(controllerRot) * (grabObject.transform.position - controllerPos);
  ...
  void ReleaseObject()
  {
    ...
    {
      if (
          grabObject.GetComponent<GrabObject>().objectManipulationType
          == GrabObject.ManipulationType.Menu
      )
      {
  ...
  {
    RaycastHit[] objectsHit = Physics.RaycastAll(
        controllerPos,
    ...
    if (objectInfo && objectInstructionsLabel)
    {
      ...
      else if (grabObject)
      {
        Vector3 targetPos =
            controllerPos
            + (Camera.main.transform.position - controllerPos).normalized * 0.1f;
        objectInfo.position = Vector3.Lerp(objectInfo.position, targetPos, grabTime);
        ;
        objectInfo.rotation = Quaternion.LookRotation(
            objectInfo.position - Camera.main.transform.position
        );
    ...
    if (obj.GetComponent<GrabObject>())
    {
      ...
      switch (obj.GetComponent<GrabObject>().objectManipulationType)
      {
        ...
        case GrabObject.ManipulationType.HorizontalScaled:
          ...
          obj.transform.rotation =
              Quaternion.Euler(0, rotationOffset, 0) * Quaternion.LookRotation(newFwd);
          break;
        case GrabObject.ManipulationType.VerticalScaled:
          ...
          obj.transform.rotation = Quaternion.LookRotation(
              Vector3.up,
              Quaternion.Euler(0, rotationOffset, 0) * newUp
          );
          break;
        case GrabObject.ManipulationType.Menu:
          Vector3 targetPos =
              handGrabPosition + (handGrabRotation * Vector3.forward * 0.4f);
          Quaternion targetRotation = Quaternion.LookRotation(
              targetPos - camGrabPosition
          );
          obj.transform.position = Vector3.Lerp(
              obj.transform.position,
              targetPos,
