...
public class FlashlightController : MonoBehaviour
{
  ...
  void AlignWithHand(OVRHand hand, OVRSkeleton skeleton)
  {
    ...
    flashlightRoot.rotation = Quaternion.LookRotation(
        skeleton.Bones[6].Transform.position - skeleton.Bones[0].Transform.position
    );
  }
  ...
  IEnumerator FadeLighting(Color newColor, float sceneLightIntensity, float fadeTime)
  {
    ...
    while (timer <= fadeTime)
    {
      ...
      if (sceneLight)
        sceneLight.intensity = Mathf.Lerp(currentLight, sceneLightIntensity, normTimer);
      yield return null;
    }
  }
}
