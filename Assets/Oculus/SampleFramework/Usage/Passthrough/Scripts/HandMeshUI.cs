...
public class HandMeshUI : MonoBehaviour
{
  ...
  void Update()
  {
    ...
    if (rightHeldKnob >= 0)
    {
      Vector3 localCursorPos = knobs[rightHeldKnob].transform.parent.InverseTransformPoint(
          RfingerPos
      );
    ...
    else
    {
      ...
      {
        if (
            Vector3.Distance(RfingerPos, knobs[i].transform.position) <= 0.02f
            && leftHeldKnob != i
        )
        {
          rightHeldKnob = i;
    ...
    if (leftHeldKnob >= 0)
    {
      Vector3 localCursorPos = knobs[leftHeldKnob].transform.parent.InverseTransformPoint(
          LfingerPos
      );
    ...
    else
    {
      ...
      {
        if (
            Vector3.Distance(LfingerPos, knobs[i].transform.position) <= 0.02f
            && rightHeldKnob != i
        )
        {
          leftHeldKnob = i;
  ...
  void SetSliderValue(int sliderID, float value, bool isNormalized)
  {
    ...
    float absoluteValue = isNormalized
        ? value * (sliderEnd - sliderStart) + sliderStart
        : value;
    float normalizedValue = isNormalized
        ? value
        : (value - sliderStart) / (sliderEnd - sliderStart);
  ...
  void CheckForHands()
  {
    ...
    else
    {
      if (handsActive)
      {
        ...
        transform.position =
            (
                rightHand.Bones[20].Transform.position
                + rightHand.Bones[20].Transform.position
            ) * 0.5f;
        transform.position +=
            (transform.position - Camera.main.transform.position).normalized * 0.1f;
