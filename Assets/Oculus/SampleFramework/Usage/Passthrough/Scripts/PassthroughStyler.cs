...
public class PassthroughStyler : MonoBehaviour
{
  ...
  IEnumerator FadeToCurrentStyle(float fadeTime)
  {
    ...
    while (timer <= fadeTime)
    {
      ...
      passthroughLayer.colorMapEditorBrightness = Mathf.Lerp(
          brightness,
          savedBrightness,
          normTimer
      );
      passthroughLayer.colorMapEditorContrast = Mathf.Lerp(
          contrast,
          savedContrast,
          normTimer
      );
      passthroughLayer.colorMapEditorPosterize = Mathf.Lerp(
          posterize,
          savedPosterize,
          normTimer
      );
  ...
  IEnumerator FadeToDefaultPassthrough(float fadeTime)
  {
    ...
    while (timer <= fadeTime)
    {
      ...
      passthroughLayer.edgeColor = Color.Lerp(
          edgeCol,
          new Color(edgeCol.r, edgeCol.g, edgeCol.b, 0.0f),
          normTimer
      );
      if (timer > fadeTime)
      {
  ...
  public void GetColorFromWheel()
  {
    ...
    var toImg = new Vector2(
        localPos.x / colorWheel.sizeDelta.x + 0.5f,
        localPos.y / colorWheel.sizeDelta.y + 0.5f
    );
