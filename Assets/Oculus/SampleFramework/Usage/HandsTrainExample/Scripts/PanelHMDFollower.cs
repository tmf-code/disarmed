  ...
  public class PanelHMDFollower : MonoBehaviour
  {
    ...
    private void Update()
    {
      ...
      float headMovementSpeed =
          (_cameraRig.centerEyeAnchor.position - _prevPos).magnitude / Time.deltaTime;
      ...
      if (
          (
              (distanceFromLastMovement > _maxDistance)
              || (_minZDistance > currDiffFromCenterEye.z)
    ...
    private IEnumerator LerpToHMD()
    {
      ...
      while (Time.time < endTime)
      {
        transform.position = Vector3.Lerp(
            transform.position,
            newPanelPosition,
            (Time.time - startTime) / TOTAL_DURATION
        );
        yield return null;
      }
